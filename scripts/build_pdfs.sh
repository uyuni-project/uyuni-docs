...
# Stage each language one time, and wait for it. Every book below runs with
# STAGE=0 and depends on this: it writes entities.adoc, which 'task
# translations' removed with the rest of the translations tree, and it copies
# the English fallback modules. Staging inside each book instead would make
# every book of a language write those shared files at the same time.
