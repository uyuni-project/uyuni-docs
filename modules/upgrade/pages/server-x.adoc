...
[WARNING]
====
Migrating directly to 4.2 from an older version such as version 3.2 is not possible.
Instead it must happen in two stages:
You must first upgrade from 3.2 to 4.1, and then from 4.1 to 4.2.
For more information about migrating from 3.2 to 4.1, see https://documentation.suse.com/external-tree/en-us/suma/4.1/suse-manager/upgrade/upgrade-overview.html.
We strongly recommend that you contact SUSE Consulting to assist with this process.

For more information about migrating from 4.1 to 4,2, see xref:upgrade:server-y.adoc[].
====
