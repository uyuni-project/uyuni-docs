...
[NOTE]
====
With default configuration setttings, all zypper commands run a repository refresh every 10 minutes (see ``repo.refresh.delay`` in [path]``/etc/zypp/zypp.conf``).
If you disabled ``autorefresh``, only an explicit [command]``zypper ref`` command will refresh all repositories.
====
