...
If you want to migrate many clients to the next SP version, {suse} recommends to use {productname} API calls.
...
Now we can iterate over the system IDs and call [systemitem]``listMigrationTarget`` for every system and check if the wanted target is part of the available targets.
If yes, we call [systemitem]``system.scheduleSPMigration``, otherwise we skip the system.

Here is a template that you can adapt for your environment:
