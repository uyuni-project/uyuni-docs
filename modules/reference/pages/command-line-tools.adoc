...
// This file is 1000 lines long. Might need to split it up into sub-directories. --LKB 2020-10-01
...
====
When a configuration file is deployed via {productname}, a backup of the previous file including its full path is stored in the [path]``/var/lib/rhncfg/backups/``.
...
This tool allows {productname} administrators to enable or disable specific modes of allowable actions such as:
...
Once a mode is set, your system is ready for configuration management through {productname}.
...
The Configuration Client (mgrcfg-client) is installed on and run from an individual client system to gain knowledge about how {productname} deploys configuration files to the client.
...
To view the differences between the config files deployed on the system and those stored by {productname}, issue the command:
...
To quickly determine if client configuration files are different from those associated with it via {productname}, issue the command:
...
==  Configuring the Database (smdba)
