...
The custom HTTP headers are defined at [path]``/etc/rhn/spacewalk-repo-sync/extra_headers.conf`` file. They can be defined per "repository name", "channel label" or globally under the "[main]" section:
...
This can be particularly useful when dealing with RHUI repositories in the public cloud.
