[[ref-systems-ssm]]
= System Set Manager
:description: Learn how to effectively manage and configure client systems using the System Set Manager, a centralized tool for monitoring and controlling client settings
:revdate: 2020-09-11
:page-revdate: {revdate}
:page-author: SUSE Product & Solution Documentation Team
:page-image: https://www.suse.com/assets/img/suse-white-logo-green.svg
:page-product-name: SUSE Manager
:page-product-version: 4.3

The menu:Systems[System Set Manager] section allows you to manage clients that you have added to the system set manager.

For more information about the system set manager, see xref:client-configuration:system-set-manager.adoc[].
