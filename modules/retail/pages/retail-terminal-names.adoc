...
The [systemitem]``UniqueID`` is the first four characters of a generated machine identification number.
You can disable this behaviour by toggling the [systemitem]``Do not append unique suffix to the salt client ID`` checkbox in the xref:salt:formula-branchnetwork.adoc[].
