...
Naming schemes available are 'Hostname', 'FQDN' and 'HWType'. Naming scheme can be selected in xref:salt:formula-branch-network.adoc[].

By default, terminals are named according to 'Hostname' naming scheme with 'HWType' scheme as a fallback.


=== Naming by 'HWType' naming scheme
...
You can configure this value in the xref:salt:formula-branch-network.adoc[] settings for the branch server.
You can disable this prefix by toggling the [systemitem]``Do not prefix salt client ID with Branch ID`` checkbox in the xref:salt:formula-branch-network.adoc[].
...
Added unique ID is a requirement for successful terminal deployment. Without unique ID, subsequent terminal registrations will fail.
...
The [systemitem]``UniqueID`` is the first four characters of a generated machine identification number.
You can disable this behaviour by toggling the [systemitem]``Do not append unique suffix to the salt client ID`` checkbox in the xref:salt:formula-branch-network.adoc[].

=== Naming using the FQDN naming scheme

Terminal names that are derived from the Fully Qualified Domain Names  use this format:
...
The [systemitem]``BranchID`` is the unique identifier for the branch server that the terminal is connected to.
You can configure this value in the xref:salt:formula-branch-network.adoc[Branch Network formula] settings for the branch server.
You can disable this prefix by toggling the [systemitem]``Do not prefix salt client ID with Branch ID`` checkbox in the xref:salt:formula-branch-network.adoc[Branch Network formula].

The [systemitem]``FQDN`` is the fully-qualified domain name of the terminal.

The [systemitem]``UniqueID`` is the first four characters of a generated machine identification number.
You can disable this behaviour by toggling the [systemitem]``Do not append unique suffix to the salt client ID`` checkbox in the xref:salt:formula-branch-network.adoc[Branch Network formula].
