...
. On the {productname} container host, create file [path]``autosign_grains.conf`` and add this line:
+
----
autosign_grains_dir: /etc/salt/autosign_grains
----
+
// FIXME: also on container host, where dir does not matter?
...
. Push new files into the container using [command]``mgrctl``:
+
----
