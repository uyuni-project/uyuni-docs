[[retail-migration-intro]]
= Retail Migration
:description: Learn how to successfully migrate from previous versions of SUSE Linux Enterprise Point of Service and SUSE Manager for Retail to the newest version
:revdate: 2022-01-04
:page-revdate: {revdate}
:page-author: SUSE Product & Solution Documentation Team
:page-image: https://www.suse.com/assets/img/suse-white-logo-green.svg
:page-product-name: SUSE Manager
:page-product-version: 4.3

ifeval::[{suma-content} == true]
This section provides instructions for migrating {slepos} 11, {productname} {smr} 3.1, or {productname} {smr} 3.2 to the newest version of {productname} {smr}.
endif::[]

ifeval::[{uyuni-content} == true]
// FIXME
For migrating {productname} {smr} to the latest version, see the {productname} upgrade instructions.
endif::[]
