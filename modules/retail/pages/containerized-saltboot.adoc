...
:description: Configure your SUSE Manager for Retail environment using a containerized proxy by following these steps and understanding key concepts like Saltboot group
:revdate: 2023-12-19
...
All [systemitem]``saltboot`` deployed machines though containerized proxy will automatically become members of its [systemitem]``Saltboot group``.
...
[IMPORTANT]
====
Once POS image is build and made available on {productname} Server, it is immediately available to the {saltboot} clients as well. Image synchronization is not needed, nor available.
