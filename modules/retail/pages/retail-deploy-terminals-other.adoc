...
You can configure Saltboot to send automatically signed grains once and then delete them.
This method appends the Saltboot configuration to an existing ``initrd``.

You can also choose to keep the automatically signed grains on the Salt client.
This method includes the configuration file in the image source before the client image is built.
The auto-signed grain is then stored on the client as a regular Salt grain after booting.
...
----
. Create a file at [path]``/etc/salt/autosign_grains/autosign_key``, that contains the autosign key you specified with Saltboot:
+
----
<AUTOSIGN_KEY>
----
