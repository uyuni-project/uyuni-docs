...
You can instruct terminals to download and deploy images every time they are restarted.
This is achieved using a Salt state.

[[Warning]]
====
Re-deploying a terminal removes all data stored on the terminal hard disk, including any persistent partitions.
====
