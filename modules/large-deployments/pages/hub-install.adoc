...
. OPTIONAL: verify contents of the configuration file at  ``/etc/hub/hub.conf``
  ...
  - ``HUB_REQUEST_TIMEOUT``: maximum numbr of seconds to wait for a response when calling a Server method. In most cases, these parameter can be left at its default value.
