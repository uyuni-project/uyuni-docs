...
| Tune when            | Database operations are slow because of excessive temporary file disk I/O. To test if that phenomenon is happening, add `log_temp_files = 5120` to [path]``/var/lib/pgsql/data/postgresql.conf``, restart Postgres, and monitor Postgres log files. If lines appear containing the string `LOG:  temporary file:` then raising this parameter's value might help reducing disk I/O thus speeding up database operations.
