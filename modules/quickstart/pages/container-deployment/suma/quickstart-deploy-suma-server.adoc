...
[IMPORTANT]
====
Server deployment requires a fully qualified domain name (FQDN). If your router or network does not automatically provide one via DNS the deployment will fail. The FQDN should be similar to <host.domain.com>. 
...
For more information, see xref:installation-and-upgrade:network-requirements.adoc[].
====
...
. You will need an account and be logged in to download the ISO.
