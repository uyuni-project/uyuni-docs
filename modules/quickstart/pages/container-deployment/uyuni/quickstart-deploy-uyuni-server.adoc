...
Select one of the following installers, wait for the download to complete and move it to the default KVM image store:
...
----
cd Downloads/
sudo mv openSUSE-Leap-Micro.x86_64-Default-qcow.qcow2 /var/lib/libvirt/images/
----

== Virtual Machine installation with {leapmicro}
