...
= Troubleshooting Custom Channel without Conflicting Packages
...
. Important note: In the CLM project, it is recommend to add a filter, which excludes the problematic or conflicting packages.
  ...
  For more information about filtering, see xref:administration:content-lifecycle-examples#exclude-higher-kernel-version[].
...
* For more information about CLM, see xref:administration:content-lifecycle[].
