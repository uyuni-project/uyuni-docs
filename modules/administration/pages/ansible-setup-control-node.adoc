...
. From the {webui} under menu:Admin[Setup Wizard > Products], verify that the product [literal]``SUSE Linux Enterprise Server 15 SP5 x86_64`` channel and [literal]``SUSE Manager Client Tools for {sles} 15 x86_64`` are installed and synchronized.

. Deploy a {sles} 15 SP5 client.

. Select the client from the menu:Systems[Overview] page.
  Select menu:Software[Software Channels] and subscribe the client to the channels [literal]``SUSE Linux Enterprise Server 15 SP5 x86_64`` and [literal]``SUSE Manager Client Tools for {sles} 15 x86_64``.
+
