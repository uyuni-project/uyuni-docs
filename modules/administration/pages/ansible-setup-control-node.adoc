...
[NOTE]
====
To configure a {productname} client the Ansible software must be available for operating system of the client.
This usually means Ansible must come from the client's software distribution.
For example, Ansible is available for {sle} 15 SP6 or SP7.
====
...
. In the {productname} {webui}, navigate to the menu:Systems[Overview] page of the client.
  Select menu:Software[Software Channels] and subscribe the client to the [literal]``SUSE Linux Enterprise Server 15 SP5 x86_64``, [literal]``Systems Management Module`` and  [literal]``Python 3 Module`` channels.
...
[NOTE]
====
If you want to install a newer Ansible on a {sle} 15 SP4 or SP5 client, you must enable only the [literal]``Python 3 Module``.
====

[NOTE]
====
With newer Ansible, systems with an old Python version cannot be managed anymore.
====
