...
[NOTE]
====
The following procedure applies for {sle} 15 system only.
On some systems, for example {almalinux} 9, Ansible is available from the distribution packages; in such a case, the user can install  it manually.
====


.Procedure: Setting up Ansible Control Node on a {sle} 15 system
...
[NOTE]
====
On {sle} 15, the {productname} client tools contain the [package]``ansible`` package.
====
