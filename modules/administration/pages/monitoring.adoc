...
As mentioned above scrape targets have to be accessible from Prometheus server. In case of clients installed on cloud instances it is often the simplest and  the best approach to add required ports into security group and grant access to the monitoring server. Following ports are used by the shipped exporters:
