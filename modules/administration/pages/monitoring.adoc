...
. In the [guimenu]``Postgres Exporter`` section, in the [guimenu]``Data Source Namer`` field, enter the the string [systemitem]``postgresql://USER:PASSWORD@localhost:5432/database?sslmode=disable``, replacing ``USER`` and ``PASSWORD`` with the values of the ``db_user`` and ``db_password`` from the ``/etc/rhn/rhn.conf`` file. 
