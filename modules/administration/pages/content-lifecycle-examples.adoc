...
You can use the Appstream filter to transform modular repositories into a regular repository.
It does this by keeping the the packages in the repository and stripping away the module metadata.
...
Any module streams that the selected stream depends on will be automatically included.
...
When you build your CLM project using the btn:``Build`` button in teh {webui}, the target repository is a regular repository without any modules, that contains packages from the selected module streams.
