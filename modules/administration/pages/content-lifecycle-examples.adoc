...
. Decide which kernel version you will upgrade to.
...
If you are using {rhel}{nbsp}8 clients, you cannot access the modular repositories using the {productname} {webui}.
An alternative is to use non-modular repositories, which are provided by Appstream.
Appstream packages are designed to be compatible between all major Linux distributions.

When you search for available packages, results from Appstream repositories are included as non-modular packages, and can be used for regular package operations.

The stream filter works slightly differently to the other filters in CLM.

.Procedure: Searching for Appstream Packages
...
. Create a new filter, type the module name you require, and in the [guimenu]``Filter Type`` field, select [parameter]``Module (Stream)``.
. Leave the [guimenu]``Stream`` field blank to return both modular and non-modular Appstream packages that match your filter.
