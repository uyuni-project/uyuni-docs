...
. On the {productname} container host, at the command prompt, call the following command to run [command]``mgr-ssl-cert-setup`` inside the container and provide the certificates as parameters:
+
----
mgrctl exec -ti -- mgr-ssl-cert-setup \
  --root-ca-file=<Path_to_Root_CA_Certificate> \
...
You should similarily run [command]``mgr-ssl-cert-setup`` inside a {productname} Proxy container to replace the certificates.
To restart the Proxy, on {productname} Proxy container host, call [command]``mgrpxy restart``.
