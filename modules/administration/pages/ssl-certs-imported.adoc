...
* A certificate authority (CA) SSL public certificate.
  If you are using a CA chain, all intermediate CAs must be available as well.
...
If there is not extra parameter or option available to specify intermediate CAs, take care that all CAs - Root CA and intermediate CAs -
...
. On the {productname} Server, at the command prompt, and call the command `mgr-ssl-cert-setup` and provide the certificates as parameter:
+
----
...
Intermediate CAs can either be available in the file which is specified as 'root-ca-file' or specified by extra options `--intermediate-ca-file`.
This option can be used multiple times.

This command performs a number of tests on the provides files to test if they are valid and can be used for the requested use case.
...
If the CA needs to be replaced when there are still traditional managed clients connected to {productname}
...
Use a configuration channel to deploy the certificte files to the clients and the remote command feature to regenerate
the trust store.

After the new certificates are activated on the {productname} Server and Proxies, test if the connections are working
and actions can still be scheduled on the clients. If this is the case, the 'old' Root CA can be removed from clients.
