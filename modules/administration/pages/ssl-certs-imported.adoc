...
You should use [command]``mgr-ssl-cert-setup`` also on a {productname} Proxy container host to replace the certificates.
On {productname} Proxy,[command]``mgrpxy restart``.
