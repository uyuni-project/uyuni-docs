...
If you migrated from SUSE Manager with [command]``smdba`` to the new solution, you must disable the old funtionality and remove the old backup archives.
====

Either disable smdba before migrating from SUSE Manager 4.3 or 5.0 (recommended) or later on the migrated {productname} {productnumber} system.
...
[source,shell]
----
mgrctl exec -ti -- smdba backup-hot enable off
----
...
////
in /var/lib/pgsql/data/postgresql.conf change the following parameters:
...
* On micro container host:

* On sles container host

////

after this restart postgress (or beter the container)
