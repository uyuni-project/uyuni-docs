...
On the container host, use [literal]``mgrctl term`` to open a terminal in the container before you execute commands.
...
////
// FIXME: do we want to keep this?  With the advent of persistent storage
//        it does not look needed to me.  I think, you'd better solve
//        storage issues on the container host, not inside the container
...
//        Wouldn't the user better re-deploy the container?
