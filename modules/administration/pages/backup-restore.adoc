[[backup-restore]]
= Backup and Restore
:description: Learn how to back up and restore your SUSE Manager installation effectively by configuring database backups with the smdba tool, scheduling search indexes,
:revdate: 2024-01-08
...
* [path]``/var/run/salt/``
* [path]``/var/spacewalk/``
* Any directories containing custom data such as scripts, Kickstart or {ay} profiles, and custom RPMs.
...
Archive logs are stored at [path]``/var/lib/pgsql/data/pg_logical/`` (postgresql).
