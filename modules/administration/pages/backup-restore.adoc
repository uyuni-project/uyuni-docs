...
. Enter in the container terminal
+
----
mgrctl term
----
...
====
Ensure you have at least three times the current database size of space available for backups.
You can check your current database size by running [command]``mgrctl exec "df -h /var/lib/pgsql"``.
====


.Procedure: Create a directory for the backup (do it only once)
. Enter in the container terminal
+
----
mgrctl term
...
----
. Create a system timer file at [path]``/etc/systemd/system/server-db-backup.timer`` with the content:
+
----
[Unit]
