...
When registered, you must enable the ``Ansible Control Node`` system type in the [guimenu]``Add-on System Types`` on the [guimenu]``System Details > Properties`` page of that system.

Enabling this system type ensures that the ``ansible`` package is installed on the system by adding it in the highstate and activates the Ansible features under the [guimenu]``System Details > Ansible`` page.

As the next step, configure the paths to your Ansible inventories and playbooks on the [guimenu]``System Details > Ansible > Control Node`` page.
...
You can do so on the [guimenu]``System Details > Ansible > Inventories`` page.
...
After defining a playbook directory, you can discover playbooks on the [guimenu``System Details > Ansible > Playbooks`` page.
...
You can schedule a playbook execution from the [guimenu]``System Details > Ansible > Playbooks`` page.
