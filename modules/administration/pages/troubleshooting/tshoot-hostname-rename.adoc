...
. Re-configure the clients not managed via a proxy to make them aware of the new hostname and IP address.
    In the {salt} client configuration file [path]``/etc/salt/minion``, you must specify the name of the new {salt} master ({productname} Server):
+
----
master: <new_hostname>
----
+
. When this is changed, restart the [literal]``salt-minion`` process:
+
----
