...
During the SSL handshake, the client will check that the hostname in the server certificate matches what it expects.
...
If you are using a self-signed certificate, you will have a root CA that is signed with itself.
