...
When applied, these templates help create a set of pre-defined filters, tailored for a specific use case.
...
[IMPORTANT]
====
The second "catch-all" filter is a safeguard to make sure the assigned clients are never required a restart in the future. You can choose not to attach it to the project if this behavior is not desired as it might block other important updates.
====
...
. Click btn:[Attach/Detach Filters].
. Select the 2 filters that have the specified prefix, and click btn:[Save].
