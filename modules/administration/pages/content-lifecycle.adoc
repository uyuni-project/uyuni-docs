...
You can clone vendor channels multiple times within your organization, which results in having two or more content projects with the same source vendor channels.
...
To manually synchronize the patch later for all channels sharing the patch, navigate to menu:Software[Manage > Channels], and click the channel you want to synchronize.
You can monitor progress by navigating to menu:Repositories[Sync]. 
