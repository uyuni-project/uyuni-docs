...
----
+
You can resolve this error by specifying [literal]``reposync_timeout`` and [literal]``reposync_minrate`` configuration
values in [path]``/etc/rhn/rhn.conf``. When less than [literal]``reposync_minrate`` bytes per second (default:
[literal]``1000``) are transfered for a period of [literal]``reposync_timeout`` seconds (default: [literal]``300``) the download is aborted.
