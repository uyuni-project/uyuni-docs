...
For renewing a GPG key, see xref:administration:tshoot-sync.adoc[].
...
. The deployment for GPG keys to the clients is done via salt states. Just apply the highstate via the {productname} {webui}.
