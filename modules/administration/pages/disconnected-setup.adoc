...
It is recommended to deploy {productname} Server as a Virtual Machine (VM).
The VM image comes with all needed tools included.
For a step-by-step deployment procedure, see xref:installation-and-upgrade:container-deployment/suma/server-deployment-vm-suma.adoc[].
