...
To set up {productname} as a disconnected server deploy {productname} Server as a Virtual Machine (VM) using a provided images.
For a step-by-step deployment procedure, see xref:installation-and-upgrade:container-deployment/suma/server-air-gapped-deployment-suma.adoc[].

**IMPORTANT:** Execute the final command with the [option]``--mirrorPath`` option (replace [literal]``</media/disk>`` with your mount point):
+
----
