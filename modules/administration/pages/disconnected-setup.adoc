...
. Deploy the {productname} {productnumber} Server as a Virtual Machine.
  For a step-by-step procedure, see xref:installation-and-upgrade:container-deployment/suma/server-deployment-vm-suma.adoc[].
  **IMPORTANT:** Execute the final command with the [option]``--mirrorPath`` option:
+
----
