...
* *Principle of Least Privilege:* Granting only the necessary access for users to perform their tasks.
...
Understanding the following core concepts is crutial for effective RBAC management:
...
Refer to xref:reference:users/users.adoc#administrator-roles[] for a complete list of predefined roles and their descriptions.
...
A comprehensive list of namespaces and their descriptions can be retrieved by making a call to the ``access.listNamespaces`` API method. Refer to {productname} API documentation for detailed information, including request and response formats.


[[managing-rbac]]
== Managing RBAC

Managing RBAC roles and permissions is currently only possible through the API. To assign roles to users via the web UI, refer to xref:reference:users/users.adoc[].
