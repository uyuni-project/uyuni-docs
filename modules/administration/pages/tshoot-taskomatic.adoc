...
If Taskomatic is still running, or if the process has crashed, package updates can seem available in the {webui}, but will not appear on the client, and attempts to update the client will fail.
In this case, the [command]``zypper ref`` command will show an error like this:
