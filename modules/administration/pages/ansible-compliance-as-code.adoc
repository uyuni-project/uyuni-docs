...
Packages, channels and scripts are different for each operating system and distribution. Examples are listed below in the <<scripts-for-os-types>> section.



=== Run the Bash script on single systems as a remote command

Option 1: Run the Bash script as a remote command (for single systems).
