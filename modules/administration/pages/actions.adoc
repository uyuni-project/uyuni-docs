...
[IMPORTANT]
====
For transactional update systems, please consider that an action chain is executed inside a single snapshot, until there's a reboot action. This can cause some limitations. and a link to the section where reboot (and warn) on SLE Micro is explained.
====
