...
. In the [guimenu]``Path to XCCDF Document`` field, enter the path to the XCCDF content file on the client system.
...
For example, use ``united_states_government_configuration_baseline`` to use the USGCB.
