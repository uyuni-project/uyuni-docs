...
After the authentication with the IdP using the user ``orgadmin`` is successful, you are logged in into {productname} as the ``orgadmin`` user, provided that the ``orgadmin`` user exists in {productname}.
