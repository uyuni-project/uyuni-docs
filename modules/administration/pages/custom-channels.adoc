...
If remote channels and repositories are signed with GPG keys, you can
import and trusting these GPG keys.
For example, execute the [command]``spacewalk-repo-sync`` from the command line on the {productname} Server; the {webui} does not offer this feature:

----
/usr/bin/spacewalk-repo-sync -c <channellabelname> -t yum
----

The underlying [command]``zypper`` call with import it, if the key is available.
