...
``uyuni_suite``::
is mandatory to use this way of specifying apt source, otherwise original approach is used
``uyuni_component``::
is optional.
If used it can specify only one component.
It is not possible to list the components (apt source entry allows to specify multiple components, but for Uyuni it's not possible and we need to create distinct repo for each component)
``uyuni_arch``::
is optional.
If omitted the arch name is calculated with SQL query for the channel from the DB, or specify uyuni_arch explicitly if it doesn't match the arch of the channel (shouldn't happen in real life, but debian distros have some mess in arch naming)
