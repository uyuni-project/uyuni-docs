...
If you do not install them, images can still build but the package and product list are not available in the {webui}.
====
...
Build host:::
The build host that pulls and inspect the image.
...
* If the [package]#python# and [package]#python-xml# packages are not installed in your images during the build process, reporting of installed packages or products fails.
  This results in an ``unknown`` update status.
