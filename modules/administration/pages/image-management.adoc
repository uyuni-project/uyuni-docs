...
+
. Navigate to the menu:System Details[Software > Software Channels] tab, enable required software channels depending on the build host version.
+
...
[IMPORTANT]
====
Build server cannot run any form of automounter during the image building. Make sure Gnome session is not running under root user if used.

If automounter is running, image build will finish successfully, however checksum of image will be different and will fail later.
====
