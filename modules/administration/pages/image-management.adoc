...
. Enter [guimenu]``Kiwi options`` if needed. If the Kiwi configuration files specify mutiple profiles, use ``--profile <name>`` to select the active one. For other options see Kiwi documentation.
