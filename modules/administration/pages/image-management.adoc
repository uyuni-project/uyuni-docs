...
You can render /etc/gitconfig from pillar data (for example expose the proxy in a build_server:git_proxy pillar or custom_info field) so build hosts are configured automatically. Example Salt state:

```yaml name=salt/kiwi_build_gitproxy.sls
/etc/gitconfig:
  file.managed:
    - user: root
    - group: root
    ...
    - contents: |
        [http]
            proxy = {{ salt['pillar.get']('build_server:git_proxy', 'http://proxy.example.com:3128') }}
```
