...
On a production system the ratio of physical memory to VDISK needs to be evaluated based on the number of clients you intend to install.
====

You need an additional disk for database storage.
This should be a [systemitem]``zFCP`` or [systemitem]``DASD`` device as these are preferred for use with [systemitem]``HYPERPAV``.
...
You need to ensure you have sufficient disk storage for {productname} before running [command]``yast2 susemanager_setup``.
...
{suse} technical services are not able to provide support for systems suffering from low disk space conditions.
====
...
The z/VM guest you want to run {productname} from requires a static IP address and hostname before you begin, as these cannot easily be changed after initial installation.
