...
[NOTE]
====
The recommended way of installing {susemgr} is from the {sle} installation media and the Unified Installer.
In case of installing {susemgr} in a public cloud where {susemgr} image is not available, it is possible to change a {sles}{nbsp}{sles-version}{nbsp}{sp-version} into a {susemgr}{nbsp}{productnumber}.
For steps how to do that follow the instructions at installation:install-vm.adoc[].
====
