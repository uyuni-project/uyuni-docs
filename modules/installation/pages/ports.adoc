...
You do not need to open all of these ports.
...
In most cases, you do not need to adjust these ports.
...
When a TCP connection request is received, the sender chooses one of these ephemeral port numbers to match the destination port.
...
For example, if you have {ubuntu} clients, you need to be able to access the {ubuntu} server.
