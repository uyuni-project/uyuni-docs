...
After the initial setup is complete, signing in takes you the menu:Home[Overview] section.
...
You need to check these clients.
...
Autoload::
If you click the name of a menu entry, the first available page of that menu entry is loaded and displayed automatically.
...
icon:sync[role="none"]
The circling arrows icon indicates that applying a patch requires a reboot.

icon:archive[role="none"]
The archive box icon indicates that applying the patch has an effect on package management.
