...
.Procedure: Configuring Salt Proxy After Replacement
. Ensure you have installed, reactivated, and re-reregistered the new proxy.
. On the proxy, at the command prompt, check the machine ID, and make a note of the ID for later:
+
----
cat /etc/machine-id
----
. In the {webui}, navigate to menu:Salt[Keys], locate the Salt key associated with the proxy, and click btn:[delete].


When you have completed all the preparation steps, register the proxy with a bootstrap script as described in xref:installation:proxy-registration.adoc[].
