...
Either, on the server, select the [package]``patterns_uyuni_proxy`` package for installation, or make sure the [path]``uyuni_proxy`` pattern is installed using the following command on the proxy as root:
...
. On the {productname} Server, run [command]``configure-tftpsync.sh`` to configure the upload to the {productname} Proxy:
+
