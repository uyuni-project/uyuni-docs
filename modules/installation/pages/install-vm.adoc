...
A kernel virtual machine (KVM) combined with Virtual Machine Manager (_virt-manager_) is used as a sandbox for this installation.
...
. Click btn:[Add Hardware] to create three new virtual disks with these specifications.
    The disks are partitioned and mounted in <<proc.jeos.susemgr.prep>>.
+
