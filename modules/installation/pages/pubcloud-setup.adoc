...
However, you must have the correct {productname} and {sle} entitlements to receive updates before you can log in.

For most bring your own service (BYOS) images, these entitlements are provided by {suse}, and can be obtained using the {scc}.
If you are using an on demand image, you need to do some extra preparation before you can register to {scc}.

When you have registered, all {sle} modules are activated.
You can then activate the public cloud module.



.Procedure: Preparing On Demand Images
. Log in the on demand instance.
. At the command prompt, as root, remove the `cloud-regionsrv-client` package:
+
----
...
----
. Open the ``/etc/hosts`` file, and delete the entry that contains ``@susecloud.net``.
. Remove the ``SUSEConnect`` configuration file:
+
----
rm /etc/SUSEConnect
----
