...
. When setup is complete, click btn:[Next] to see the address of the {productname} {webui}.

. Click btn:[Finish] to complete {productname} setup.

// In the next section you create the administrator's account and synchronize with {scc}.
...
// In the next section you prepare the server for connecting the first client.
...
Before you can synchronize software repositories with SCC, you need to enter organization credentials in {productname}.
The organization credentials give you access to the {suse} product downloads.
You can find your organization credentials in https://scc.suse.com/organization.
...
. From the [guimenu]``Setup Wizard`` page select the btn:[SUSE Products] tab.
  ...
  If you previously registered with {scc}, the table is populated with a list of products.
...
After adding the channel, {productname} schedules the channel to be synchronized.
This can take a long time as {productname} copies channel software sources from the {suse} repositories located at {scc} to local [path]``/var/spacewalk/`` directory of your server.
...
You also need to open the [path]``/etc/default/grub`` file and add or edit the line [option]``GRUB_CMDLINE_LINUX_DEFAULT``.
For example:
