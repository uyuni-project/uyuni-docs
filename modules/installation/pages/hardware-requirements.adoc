...
If there is not enough space available in [path]``/var/cache/``, the proxy removes old, unused packages and replaces them with newer packages.

As a result of this behavior:

* The larger [path]``/var/cache/`` directory is on the proxy, the less traffic there is between it and the {productname} Server.
...
If you are installing a proxy, the [command]``suma-storage`` command only takes a single device name parameter and sets up the external storage location as the Squid cache.
