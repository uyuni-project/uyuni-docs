...
{productname} Proxy systems are Salt clients that are installed with the Unified Installer and registered to {productname} using bootstrap script or GUI.
...
//OM: Admonitions decluttering? Do we really need this warning here? 
//OM: Our users probably know these basic guidelines?
//OM: We should have it for server too, if it exists here.
//OM:
//OM: [IMPORTANT]
//OM: ====
//OM: When choosing a username or password for your {productname} Proxy, ensure it does not contain an ``@`` or ``:`` character.
//OM: These characters are reserved.
//OM: ====
