...
. With [systemitem]``reboot`` and [systemitem]``rebootifneeded`` two reboot strategies exist.
  Users have the option to select and enable one reboot action from them.
...
* For non-transactional {suse} clients, {productname} checks if any of the patches require reboot by using [command]``zypper -x list-patches``, and reboots if so.

* For transactional {suse} clients, {productname} checks if there is a pending transaction, and reboots if so.
...
* For the Red Hat-based clients, {productname} uses [command]``dnf -q needs-restarting -r``, and reboots if so.
