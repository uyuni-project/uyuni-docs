...
{productname} 5.1 introduced a new, rebranded set of client tools for all supported operating systems.

This change requires some manual steps when upgrading from earlier versions to 5.1 or later.
...
Channels previously named `SUSE Manager Client Tools for XYZ` (with labels such as `manager-tools-*`), used in SUSE Manager 4.3 or 5.0, are no longer available from version 5.1.
...
* Update any Content Lifecycle Management (CLM) projects, activation keys, or AutoYaST profiles that reference the old client tools channels to use the new ones.



== Migration from old to new client tools using {webui}

Following the introduction of new client tools channels, CLM projects can be updated and system channels subscriptions modified, via {webui}.


=== Prerequisites

The following steps must be completed before the migration starts:

* The product list must be updated to ensure new client tools are visible. 
  This is done by running [command]``mgr-sync refresh`` from the command line, or by navigating to menu:Admin[Setup Wizard > Products] and clicking btn:[Refresh]. 
* The new client tools products must be mirrored for all synchronized operating systems.


The following procedures describe the migrations that need to take place.

.Procedure: Updating CLM environments
[role=procedure]
____
. In the {productname} {webui}, select the target CLM project.

. Click btn:[Attach/Detach Sources].

. Select the new ManagerTools channels.

. Deselect the old SLE-Manager-Tools channels (or the equivalents for other operating systems).

+
[NOTE]
====
For SUSE Linux Enterprise Server 12, the old client tools are mandatory and cannot be removed from the assigned sources.
In this case, the new client tools are added without removing the old ones.
====

. Save the changes. 

. Rebuild and promote the environments.
____


.Procedure: Updating systems using the System Set Manager (SSM)
[role=procedure]
____
. Navigate to menu:Systems[System Set Manager].

. Select the systems to be migrated.

. Navigate to the Channels tab.

. Ensure the base channel configuration remains unchanged.

. In the channel list, select [btn]:Subscribe for the new ManagerTools channels.

. Select [literal]``Unsubscribe`` for the old SLE-Manager-Tools channels (or the equivalents for other operating systems).

+
[NOTE]
====
For some SUSE Linux Enterprise Server products, the {webui} may restrict unsubscribing from the old tools.
In this instance, subscribe to the new Client Tools only.
====

. Click btn:[Apply] and then btn:[Confirm].

____



== Migration from old to new client tools using API
...
This workflow is intended for administrators managing {productname}. 
It helps automate the process of syncing new channels and updating activation keys and CLM projects, including project promotion, through the API, eliminating the need for manual operations in the {webui}.
...
=== Synchronize new client tools channels
...
.Procedure: Detecting installed products and adding required channels
[role=procedure]
____
...
=== Update CLM projects and activation keys

Once the new client tools channels are synchronized, the next step is to update your CLM projects and activation keys so that they reference the new channels instead of the old ones.
...
.Procedure: Listing projects and attaching new tools
[role=procedure]
____
...
You can use the following API calls to automate this process.
