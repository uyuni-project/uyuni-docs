...
This change requires some manual steps when upgrading from earlier versions to 5.2.
...
Channels previously named `SUSE Manager Client Tools for XYZ` (with labels such as `manager-tools-*`), used in SUSE Manager 4.3 or 5.0, are no longer available in version 5.2.  
...
This workflow is intended for administrators managing {productname} 5.2. 
