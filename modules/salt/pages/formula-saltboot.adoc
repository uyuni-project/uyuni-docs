...
If the entered value does not contain "[systemitem]``/``", the entered value is automatically prepended by "[path]``/dev/disk/by-path/``". That means that i.e. [systemitem]``\*usb*`` is the same as [path]``/dev/disk/by-path/\*usb*``.

If selecting exact devices is preferred, disk device field can be in format of:

* symbolic names (e.g. [path]``/dev/sda``)
* by-path (e.g. [path]``/dev/disk/by-path/..``)
* by-id (e.g. [path]``/dev/disk/by-id/...``)

List of available devices is visible on the terminal if you press [systemitem]``ESC`` during waiting for key approval.
