...
Configuration options are set in the [path]``/etc/salt/master`` file or a separate configuration file [path]``/etc/salt/master.d/``.  Basic settings are:
...
{suse} strongly recommends to prefer [literal]``roots`` (local files) over [literal]``gitfs``.
