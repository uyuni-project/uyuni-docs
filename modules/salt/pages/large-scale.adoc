...
Communication from the Salt Master to all Salt minions is encrypted with a single AES key. This key is rotated whenever:
 - the salt-master process is restarted, or
 - any Salt minion key is deleted (eg. a client is deleted from {productname})

Upon rotation, all minions will have to re-authenticate to the Master next time they receive a message, and if the number of minions is high (several thousands) then this is expected to cause noticeable CPU load on the {productname} Server.

If the load caused by massive re-authentication is excessive, it is recommended to delete keys in batches, possibly in off-peak hours, not to overload the {productname} Server.

More information can be found in Salt upstream documentation: https://docs.saltstack.com/en/latest/topics/tutorials/intro_scale.html#too-many-minions-re-authing https://docs.saltstack.com/en/getstarted/system/communication.html
