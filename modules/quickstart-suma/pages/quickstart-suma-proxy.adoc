[[quickstart-proxy-server]]
= Quick Start SUMA Proxy 

This guide shows you how to install and configure a single {susemgr} Server for x86-64 architecture from the SUSE Linux Enterprise installation media with the Unified Installer.
...
//+
// old comemnt The registration server offers update repositories.
...
//=== Setting up {productname} with YaST
...
//OM: . What comes next? (Activation keys, bootstrapping clients)
