...
The proxy is installed as a feature on openSUSE Leap.
Thus it is a {productname} client, but is designated as a proxy server during setup and registration.
...
You can synchronize the proxy software it to your {productname} Server.
This is also known as mirroring.
// Probably better move this statement to a later section:
On the client, use [command]``zypper` or on {productname}, the {webui} to install the proxy software on openSUSE Leap.
...
If not already done install an openSUSE Leap system.
You can use a physical or virtual machine running openSUSE Leap to install the {productname} Proxy.
Configure a resolvable fully-qualified domain name on the openSUSE Leap system before you begin, to ensure that it is accessible across the network.
...
Prepare the proxy installation settings on the server and register the openSUSE Leap system as an {productname} client.



.Procedure: Registering the openSUSE Leap system
. On the server, create an activation key with openSUSE Leap as a base channel and the proxy and the other channels as child channels.
...
+
* Either on the client, use [command]``zypper``
----
zypper in patterns-uyuni_proxy
----
* Or on the {productname} Server, use the {webui}.
  Activate the details tab of the client, and click menu:Software[Packages] and sschedule [package]``patterns-uyuni_proxy`` for installation.
. Reboot the proxy.
...
When you have prepared the proxy, you can use a supplied interactive script to complete proxy setup.
