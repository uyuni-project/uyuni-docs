...
The proxy is installed as a server extension on openSUSE Leap.
It is a {productname} client, and designated as a proxy server during setup and registration.
...
If not already done, first install an openSUSE Leap system on a physical or virtual machine.
Configure a resolvable fully-qualified domain name on the openSUSE Leap system before you begin with installing the proxy software.
That way you ensure the proxy is accessible across the network.



.Procedure: Installing openSUSE Leap
...
* Or on the {productname} Server, use the {webui}.
  Activate the details tab of the client, click menu:Software[Packages > Install], and schedule [package]``patterns-uyuni_proxy`` for installation.
. Reboot the proxy.
