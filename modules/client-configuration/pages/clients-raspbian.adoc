...
Version arm64 also requires the following channels:
...
Version armhf also requires the following channels:
...
For {raspbian} repositories, only the metadata is signed. Therefore importing a GPG key for the software channel is not needed.
