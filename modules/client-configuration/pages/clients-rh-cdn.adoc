...
[NOTE]
====
To check the [guimenu]``Enable GPG Check`` field for the Red Hat Custom Channels, populate the [guimenu]``GPG Key URL`` field using the file URL pointing to the GPG key stored in the directory [systemitem]``/etc/pki/rpm-gpg`` of the Red Hat minion.
