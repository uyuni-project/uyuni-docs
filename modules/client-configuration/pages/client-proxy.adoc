...
It schedules salt state action, which modifies master: option
in minion configuration (susemanager.conf) to point to the new proxy.
Then it restarts the minion.

Note: changing master: by manual editing of the minion configuration has
the same effect and is supported too.

When the minion restarts and reconnects via new proxy, the server updates the proxy
path in the database and schedules another action for refreshing channel URLs.



==== Salt SSH Push Clients

The proxy path is updated immediately in the database and new action
for refreshing channel URLs is scheduled.
