...
. Prepare the Dhcpd server, see xref:client-configuration:autoinst-pxeboot.adoc#prepare-the-dhcp-server[Prepare the Dhcpd Server].
...
.Example: Configuring the ISC Dhcpd server
...
.Example: Configuring the ISC Dhcpd Server using the Dhcpd formula
...
There is no need to synchronize with {productname} 5.0 proxies.
