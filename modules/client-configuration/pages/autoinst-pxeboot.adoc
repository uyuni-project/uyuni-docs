...
During a PXE boot network installation:
...
image::cobbler_menu.png[scaledwidth=80%]
...
In the installation menu, the first choice is [guimenu]``Local boot``.
If you select this, the boot process continues from the local disk drives.
This option is selected automatically if no distribution was selected after a certain time.
This is a security measure to prevent from starting an autoinstallation if no human operator is there to select one of the distributions.

Alternatively, the installation can start automatically from one of the distributions without manual intervention.
This is called "unattended provisioning".

The "bare metal" functionality is a variant of unattended provisioning where the bootloader file only registers the client at the {productname} Server and does not start the installation.
...
. Prepare an autoinstallation distribution, see xref:client-configuration:autoinst-distributions.adoc[Autoinstallation Distributions].
. Prepare an autoinstallation profile, see xref:client-configuration:autoinst-profiles.adoc[Autoinstallation Profiles].
. Reboot the client, and chose a distribution to install if needed.
...
* For fully unattended provisioning, see xref:client-configuration:autoinst-unattended.adoc[Unattended Provisioning].
