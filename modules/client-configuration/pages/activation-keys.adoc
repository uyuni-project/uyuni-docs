...
=== Salt clients
After creating a reactivation key, you can use it as the ``management_key`` grain in [path]``/etc/salt/minion.d/susemanager.conf``. For example:
...
You can now restart the [command]``salt-minion`` to apply the reactivation key.
