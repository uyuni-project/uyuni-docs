...
. In the [guimenu]``Key`` field, enter a name for the activation key.
    For example, ``SLES15-SP3`` for {sles}{nbsp}15{nbsp}SP3.
+
[IMPORTANT]
====
...
. In the [guimenu]``Base Channels`` drop-down box, select the appropriate base software channel, and allow the relevant child channels to populate.
    For more information, see xref:reference:admin/setup-wizard.adoc#vle.webui.admin.wizard.products[] and xref:administration:channel-management-custom-channels.adoc[].
...
----
sles15-sp3-web_server-room_129-x86_64
----

----
sles15-sp3-test_packages-blg_502-room_21-ppc64le
----
