...
When you have created a reactivation key, you can use it with a bootstrap script, with Salt, or the [command]``rhnreg_ks`` command line utility.

For more information about bootstrap scripts, see xref:client-configuration:registration-bootstrap.adoc[].

With Salt, you set a grain on the Salt client in [path]``/etc/salt/minion.d/susemanager.conf``:

----
grains:
    susemanager:
        management_key: <reactivation-key>
----

The [command]``rhnreg_ks`` command is only available for traditional clients.
It re-registers the client and restores its {productname} settings.
Only on traditional clients you can combine reactivation keys with activation keys to aggregate the settings of multiple keys for a single system profile.
For example:
