[[activation-keys]]
= Activation Keys
:description: Learn how to create and manage activation keys for SUSE Manager clients, ensuring correct software entitlements and channel connections
:revdate: 2024-03-08
...
. In the [guimenu]``Base Channels`` drop-down box, select the appropriate base software channel, and allow the relevant child channels to populate.
    For more information, see xref:reference:admin/setup-wizard.adoc#vle.webui.admin.wizard.products[Setup Wizard] and xref:administration:custom-channels.adoc[].
