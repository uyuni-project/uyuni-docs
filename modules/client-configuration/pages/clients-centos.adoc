...
. On the {productname} Server host, from the command prompt, as root, add the [systemitem]``sle-module-development-tools`` module:
+
----
transactional-update register --product sle-module-development-tools/15.5/x86_64
----
. On the container host, install the errata service dependencies:
+
----
...
+
----
mgrctl exec -- vi /etc/rhn/rhn.conf
----
. Restart Tomcat:
+
// FIXME: Or better restarting the server with 'mgradm restart'?
+
----
mgrctl exec -- systemctl restart tomcat
----
. On the container host or a separate machine, save the following shell code as your errata script with the name [path]``/usr/local/bin/cent-errata.sh``, and edit the repository details as required:
+
----
#!/bin/bash
...
. On the machine with the [path]``cent-errata.sh`` script, set up a cron job to run the script daily:
+
----
