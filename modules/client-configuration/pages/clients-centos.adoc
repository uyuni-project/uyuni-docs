...
----
. On the container host, save the following shell code as your errata script with the name [path]``/usr/local/bin/cent-errata.sh``, and edit the repository details as required:
+
----
#!/bin/bash
...
. On the container host, set up a cron job to run the script daily:
+
----
