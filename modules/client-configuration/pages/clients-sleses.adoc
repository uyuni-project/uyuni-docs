...
== Migrate Extended Life Clients to {sll}

If an extended life (EL) client such as RHEL and all clones (like {centos}, {almalinux}, {rocky}, and {oracle}) is already registered as a minion on {productname} and users wanted to migrate it to {sll} they can use a re-activation key to apply the activation key that drives the migration.

For more information about re-activation keys, see xref:client-configuration:activation-keys.adoc#reactivation[].

This procedure was tested on EL9, but for other versions, the workflow is the same.
...
////
// FIXME
// Do we need a procedure as follows? (ATM, it is comment and does not
// appear in the documentation.)
// What needs to be adjusted?
.Procedure: Migrating an EL client to {sll} with a bootstrap script
. In the {webui}, create a reactivation key for the existing client.
  On the [guimenu]``System Details`` tab of the existing client click [guimenu]``Reactivation``.
  Click [guimenu]``Generate New Key``, and make a note of the new key.
. In the {webui}, navigate to menu:Salt[Keys], locate the Salt key associated with the existing client, and click btn:[delete].
. Register the client anew with a bootstrap script as described in xref:installation-and-upgrade:proxy-registration.adoc[].
  In the bootstrap script, set the reactivation key with the [systemitem]``REACTIVATION_KEY`` parameter or run the script on the client and pass the re-activation key as an environment variable:
+
----
REACTIVATION_KEY=<KEY> ./bootstrap_liberate9.sh
----
