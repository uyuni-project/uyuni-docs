...
If an EL client such as RHEL and all clones (like centos, Alma Linux, Rocky Linux, and Oracle Linux) is already registered as a minion on {productname} and users wanted to migrate it to {ssl} they can use a re-activation key to apply the activation key that drives the migration.

For more information about re-activation keys, see xref:client-configuration:activation-keys#reactivation[].
