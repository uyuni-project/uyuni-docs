...
// FIXME: 2024-05-08, ke: Do we have to install the python3 pkg on rhel 9 here, too?
.Procedure: Using Package Locks
. On the {rhel} 7 systems, install the [package]``yum-plugin-versionlock`` package as [systemitem]``root``.
  ...
  On the {rhel} 9 systems, install the [package]``python3-dnf-plugin-versionlock`` package as [systemitem]``root``.
