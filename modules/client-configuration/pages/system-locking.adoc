...
.Procedure: Using Package Locks
. On the {rhel} 7 systems, install [package]``yum-plugin-versionlock``, and on the {rhel} 8 or 9 systems, install [package]``python3-dnf-plugin-versionlock`` package as [systemitem]``root``.
