...
Salt clients now use GPG key information entered for a software channel to manage trusted keys.
When a software channel with GPG key information is assigned to a client, the key is trusted once the channel is refreshed or the first package is installed from the channel.
...
A software channel cannot be used when a GPG key is not trusted.
Assigning a channel to a client depends on if its GPG key has been trusted.
====
