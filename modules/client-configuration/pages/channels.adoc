...
For {sles} clients the installer updates channel is installed by default when you add a product that contains them, and are enabled when you first synchronize the client.

For all other {sle} variants, including {sle} for SAP,  you must add the installer updates channel manually.
To do this, clone the installer updates channel that belongs to the appropriate base channel, and assign it to the client.
