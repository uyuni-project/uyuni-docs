...
== Migration from old to new client tools

Following the introduction of new client tools channels, existing clients must be migrated to the new channel structure.
Content Lifecycle Management (CLM) projects must also be updated, and system channels subscriptions modified.
...
. Select the new ManagerTools channels (*CHECK NAMES?*).
