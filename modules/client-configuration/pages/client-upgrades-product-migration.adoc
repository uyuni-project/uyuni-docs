...
Content Lifecycle Management (CLM) projects must also be updated, and system channeli subscriptions modified.

=== Migrating from old to new client tools

==== Prerequisites

The following steps must be completed before the migration strats:

* The product list must be updated to ensure new client tools are visible. 
  This is done by running [command]``mgr-sync refresh`` from the command line, or by navigating to menui:Admin[Setup Wizard > Products] and clicking btn:[Refresh]. 
...
. Select the new >>>>CHECKNAME>>>ManagerTools channels.
