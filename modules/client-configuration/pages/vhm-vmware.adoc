...
. Download the CA Certificate from your VMWare installation.
    You can do this from the {productname} server buy executing [command]``wget --no-check-certificate https://vcenter.domain.com/certs/download.zip``, where ``vcenter.domain.com`` is the fully-qualified hostname for the VMWare installation.
...
For more information on how to download and install vCenter Server root certificates, see the VMWare article 2108294: https://kb.vmware.com/s/article/2108294
