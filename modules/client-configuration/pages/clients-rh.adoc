...
This guide covers clients registered to {redhat} CDN or Red Hat Update Infrastructure (RHUI) repositories.
You must have at least one system registered to the CDN or RHUI, with an authorized subscription for repository content.
...
[NOTE]
====
If your {redhat} client is using Red Hat Update Infrastructure (RHUI) repositories, you need to use a different set of CA and entitlement certificates. 
For example:

* cert: /etc/pki/rhui/product/PRODUCT.crt
* key: /etc/pki/rhui/PRODUCT.key
* CA cert: /etc/pki/rhui/cdn.redhat.com-chain.crt
====
...
[IMPORTANT]
====
For RHUI repositories, you must manually add the [systemitem]``X-RHUI-ID`` and [systemitem]``X-RHUI-SIGNATURE`` required HTTP headers to the [path]``/etc/rhn/spacewalk-repo-sync/extra_headers.conf`` configuration file.
Without these headers, the [command]``spacewalk-repo-sync`` command will not be able to synchronize content from the RHUI repositories.
...
Use this example to calculate the the required values for the HTTP headers from a {rhel} instance running on a public cloud:
...
[IMPORTANT]
====
In order to allow [command]``spacewalk-repo-sync`` to sync content from RHUI repositories, we need to manually add the required [systemitem]``X-RHUI-ID`` and [systemitem]``X-RHUI-SIGNATURE`` required HTTP headers to [path]``/etc/rhn/spacewalk-repo-sync/extra_headers.conf`` file:
...
An example about how to calculate those values for the required HTTP headers from a {rhela} instance running on the Public Cloud:
