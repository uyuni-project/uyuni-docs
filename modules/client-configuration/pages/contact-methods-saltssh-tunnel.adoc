...
// FiXME: only here with tunnel, otherwise move to contact-methods-saltssh.adoc
[IMPORTANT]
====
...
To cover all needed services, it is the best to restart [command]``spacewalk-service`` as root:
