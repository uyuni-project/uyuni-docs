...
``== Salt 3000 to Salt Bundle Migration


=== Switch {sles} 12 or {centos} 7 minions (Salt 3000 EOL) to Salt Bundle.
...
. Apply [literal]``util.mgr_switch_to_venv_minion`` with no pillar specified first.
  This will result in the switch to [package]``venv-salt-minion`` with copying configuration files in [path]``etc``.
...
=== Switch {sles} 12 or {centos} 7 SSH minions (Salt 3000 EOL) to Salt Bundle.

.Procedure: Enabling the Salt Bundle with Salt SSH support

. Include Salt Bundle in the bootstrap repository for {sles} 12 or {centos} 7 by calling [command]``mgr-create-bootstrap-repo`` explicitly specifying [package]``venv-salt-minion`` as an additional package. 
+
----
...
[NOTE]
====
In order to add the [package]``venv-salt-minion`` package to the bootstrap repository it has to be created first. In case it has not been created, see  xref:client-configuration:bootstrap-repository.adoc#_prepare_to_create_a_bootstrap_repository[Prepare to Create a Bootstrap Repository].
====

. Specify in [path]``/etc/rhn/rhn.conf``:
+
----
...
. Restart the services using [command]``spacewalk-service restart``.
