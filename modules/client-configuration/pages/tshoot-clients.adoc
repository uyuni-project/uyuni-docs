...
== DNS resolution on minions incorrectly shown as down 

If the minion is up and running, but some actions, i.e. package refresh, apply states, etc, are marked as failed with the message:

----
Minion is down or could not be contacted.
----

try rescheduling the action.
If rescheduling succeeds, one of the reasons behind this situation is a problem with DNS configuration. 

When the Salt minion is restarted, or in case the grains are refreshed, the minion calculates its FQDNS grains, and it is unresponsive until the grains are proceesed.
When a scheduled action on {productname} Server is going to be executed, {productname} Server performs a ``test.ping`` before the actual action to ensure the minion is actually running and the action can be triggered. 

By default, {productname} Server waits for 5 seconds to get the response from ``test.ping`` command.
If the response is not received within 5 seconds, then the action is set to fail with the message that the  minion is down or could not be contacted.

In order to correct this, you should fix DNS resolution on the minion, so the minion does not get stuck for 5 seconds while solving its FQDNs.

In case this is not possible, a workaround would be to increase the value for command ``java.salt_presence_ping_timeout`` in ``/etc/rhn/rhn.conf`` file on the {productname} erver to a value higher than 4.
...
[NOTE]
====
Be mindful that increasing this value will cause {productname} Server to take longer to check if a minion is unreacheable or unresponsive, causing the {productname} Server to be a bit slower or less responsive overall.
====
