...
In case this is not possible, a workaround would be to increase the value for command ``java.salt_presence_ping_timeout`` in ``/etc/rhn/rhn.conf`` on the {productname} erver to a value higher than 4.
