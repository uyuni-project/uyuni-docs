...
{centos} 6 is now at end-of-life, and the ISO images provided in the client repository for this operating system are out of date.
...
If you need to bootstrap new {centos} 6 clients you can remove the old repositories, and create a new repository to provide the correct packages.



.Procedure: Troubleshooting Bootstrapping a New {centos} 6 Client
. On the {centos} 6 client, at the command prompt, locate the ``CentOS-Base.repo`` file provided by the {centos} 6 ISO.
In most cases, it is located at `/etc/yum/repos.d`.
. Open the repository file in your preferred text editor, and locate the `baseurl` line, which points to a ``mirror.centos.org`` URL.
For example:
+
----
...
----
+
Edit this line to be a ``vault.centos.org`` URL.
For example:
+
----
