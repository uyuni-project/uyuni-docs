...
For the initial registration from {webui}, all salt minions are initially using Salt SSH.

Because of its nature, Salt SSH minions do not report errors back to the Server.

However the Salt SSH minions have a log available at `/var/log/salt-ssh.log` that can be inspected for errors.
