...
== Minions incorrectly shown as down due to problems with DNS resolution

Even if the minion is running, actions such as package refresh or apply states can be marked as failed with the message:
...
When the Salt minion is restarted, or in case the grains are refreshed, the minion calculates its FQDNS grains, and it is unresponsive until the grains are proceeded.
When a scheduled action on {productname} Server is going to be executed, {productname} Server performs a ``test.ping`` to the minion before the actual action to ensure the minion is actually running and the action can be triggered.
...
To correct this, you should fix DNS resolution on the minion, so the minion does not get stuck for 5 seconds while solving its FQDNs.

In case this is not possible, a workaround would be to increase the value for command ``java.salt_presence_ping_timeout`` in ``/etc/rhn/rhn.conf`` file on the {productname} Server to a value higher than 4.
...
[NOTE]
====
Be mindful that increasing this value will cause {productname} Server to take longer to check if a minion is unreachable or unresponsive, causing the {productname} Server to be a bit slower or less responsive overall.
====
