...
Having repositories assigned directly to clients not served by {productname} is not usual and can create problems.
Therfore bootstrapping via Salt disable all local repositories at the beginning of the bootstrap process.

Also later on every use of the channel state, for example when executing a Highstate or a Package installation,
all locally assigned repositories are disabled again.
