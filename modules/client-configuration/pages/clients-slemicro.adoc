...
// FIXME: add this:
////
To register a new client to Uyuni, first synchronize software channels for the new client's operating system and architecture by using either Web UI or CLI.
# Then, for Uyuni only
Then, synchronize the client tool repository for the newly added operating system by using CLI.
////
