...
By default, the reboot method is set to [option]``auto``.
[command]``rebootmgrd`` will be used to reboot the system according to the configured policies if the service is running, otherwise systemctl reboot will be called.

When bootstraping a transactional system on {productname}, [systemitem]``systemd`` will be configured as the reboot method ([literal]``REBOOT_METHOD``), if the system is in its default configuration.
The idea behind this strategy is to put Suma on control of the reboot action, considering that systemd should perform the reboot immediately.
