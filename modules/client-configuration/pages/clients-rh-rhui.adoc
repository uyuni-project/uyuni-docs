...
This section contains information about using {redhat} update infrastructure (RHUI) to register Salt clients running {rhel} operating systems.
...
[IMPORTANT]
====
The PAYG connection regular check with the client to get the latest authentication data. It is important that the client stay running and is regular updated.
...
[NOTE]
====
Update the a {redhat} 7 instance before connecting it.
===
...
[NOTE]
====
A {redhat} 9 instance need to be configured with crypto policy `LEGACY` to be able to connect it.
Execute ``sudo update-crypto-policies --set LEGACY`` to do this.
====
...
Complete the setup process with with <<proc-finishing-rhui-connecting>>.
...
| {redhat} 7 | rhel7-pool-uyuni-x86_64
| {redhat} 8 | rhel8-pool-uyuni-x86_64
| {redhat} 9 | rhel9-pool-uyuni-x86_64
