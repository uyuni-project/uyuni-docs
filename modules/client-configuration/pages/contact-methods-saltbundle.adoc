...
To enable the Salt Bundle with Salt SSH support the following parameter needs to be specified in [path]``/etc/rhn/rhn.conf``:
----
web.ssh_salt_pre_flight_script = /usr/share/susemanager/salt-ssh/preflight.sh
----
To prevent using salt thin on managing Salt SSH actions the following parameter needs to be specified in [path]``/etc/rhn/rhn.conf``:
----
