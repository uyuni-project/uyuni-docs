...
Before bootstrapping new client you need to include [package]``venv-salt-minion`` to the bootstrap repository relevant to the client with explicitly specifying [package]``venv-salt-minion`` as an additional package on calling [command]``mgr-create-bootstrap-repo``.
----
...
----
Create the extra drop-in configuration file for Salt Master [path]``/etc/salt/master.d/ssh-preflight.conf``:
----
