...
When creating the bootstrap repository for SUSE Liberty Linux 9 the user must select the Salt bundle contact method.


* With the [command]``mgr-bootstrap`` command use the [option]``--force-bundle`` parameter, or

* Modify the bootstrap script manually and set:
----
