...
The ``spacewalk-utils`` package is an additional package for {productname} that can provide additional services and features.

In {productname}{nbsp}4.0 and earlier, the ``spacewalk-utils`` package contained a mixture of supported and unsupported tools.
In {productname}{nbsp}4.1 and later, the package has been split into two, with different levels of support.

The ``spacewalk-utils`` package in {productname}{nbsp}4.1 and later is fully supported by {suse}, and contains these tools:

[[spacewalk-utils]]
[cols="1,1,1", options="header"]
.Spacewalk-utils Tools
|===
