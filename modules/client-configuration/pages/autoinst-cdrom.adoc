[[autoinst-cdrom]]
= Installation via a CD-ROM or a USB Key

For clients that are not yet registered to {productname}, and if network boot over PXE is not an option, a bootable CD-ROM a USB key can be used to install the system.
