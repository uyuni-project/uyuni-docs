[[autoinst-cdrom]]
= Autoinstallation via a CD-ROM

For clients that are not yet registered to {productname}, and if PXE booting is not an option, a bootable CD-ROM can be used to install the system.

One option to prepare such a CD-ROM is to use Cobbler.
For information about using Cobbler to prepare a CD-ROM, see xref:client-configuration:autoinst-cdrom.adoc#build-iso-with-cobbler[Build an ISO Image With Cobbler].

The other option is to use mechanisms specific to the distribution:
* For {suse} systems, prepare a CD-ROM using Kiwi.
...
In order to prepare a CD-ROM with Cobbler, you need to prepare a distribution and a profile, similar to using PXE boot.
For information about creating a distribution, see xref:client-configuration:autoinst-distributions.adoc[Autoinstallation Distributions].
