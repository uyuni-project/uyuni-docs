...
. Open [path]``bootstrap-sles15.sh`` for modification.
    ...
    If ``exit 1`` exists in the file, comment it out by typing a hash sign at the beginning of the line.
...
. If you used the script to register Salt client, you can open {productname} {webui} and navigate to menu:Salt[Keys] where you can accept the client key.
