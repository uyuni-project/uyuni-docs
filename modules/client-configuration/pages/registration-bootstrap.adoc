...
// 2021-08-26, ke: do we want to uncomment this now?
== Package Locks

[NOTE]
====
Package locks can be used on traditional and Salt clients that use the Zypper package manager.
Package locks can also be used on some other Salt clients.
On {rhel}-like systems (version 7), you must have the [package]``yum-plugin-versionlock`` package installed.
// The feature is not currently supported on ...
====
...
.Procedure: Using Package Locks on CLients with the Zypper Package Manager
