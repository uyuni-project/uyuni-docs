...
[NOTE]
====
This issue happens with self-installed versions of {ubuntu} since if the default user has been granted with administrative privileges
during installation time, it will requiere a password to perform privilege escalation using [command]``sudo``.
With cloud instances this normally doesn't happen since [command]``cloud-init`` creates automatically a file under [path]``/etc/sudoers.d`` 
which grants privilege escalation through [command]``sudo`` without the need of a password.
====
