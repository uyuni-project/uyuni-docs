...
A warning will be shown in the UI that the system is locked. The UI and API will allow the user to schedule any action but this action will fail as long as the system is locked.

The system lock can be enabled or disabled from the System Lock formula. When the system lock is disabled, all operations are permitted. 
This System Lock formula is enabled automatically if {caasp} is detected on the node.
