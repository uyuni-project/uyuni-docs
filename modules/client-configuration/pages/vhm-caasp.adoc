...
You need the current keys, and keys for any clients that you was to use in the future.
...
// For example?
. Type the path passwordless SSH key you want to use, or to the ``ssh-agent`` socket.
...
The available nodes includes only nodes that are registered to {productname}, are not management nodes, and are not currently part of any cluster.
