...
This means no operations will be allowed on the node. However the user will be able to view the patch level status of the node.

The system lock can be enabled or disabled from the System Lock formula.
This formula is enabled automatically if {caasp} is detected on the node.
