...
Before you begin, ensure you have the {debian} product enabled, and have synchronized the {debian} channels for {scc}:

You can do this from {webui} or using command prompt, at your choice.

For {debian} 9:

* From the {webui}, add [systemitem]``Debian 9`` and [systemitem]``SUSE Linux Enterprise Client Tools Ubuntu 1604 amd64``.
* From the command prompt, add [systemitem]``debian-9-pool-amd64`` and [systemitem]``ubuntu-9-suse-manager-tools-amd64``.

For {debian} 10:

* From the {webui}, add [systemitem]``Debian 10`` and [systemitem]``SUSE Linux Enterprise Client Tools Ubuntu 1804 amd64``.
* From the command prompt, add [systemitem]``debian-10-pool-amd64`` and [systemitem]``ubuntu-10-suse-manager-tools-amd64``.

[NOTE]
====
The mandatory channels do not contain {debian} upstream packages.
...
* Base channel: [systemitem]``ubuntu-10-pool for amd64``
* Child channel: [systemitem]``Ubuntu-10-SUSE-Manager-Tools for amd64``
...
These examples are for {debian} 10 (bionic). 
Make sure you change the values to match the {debian} version you want to use.
...
* In the [guimenu]``Channel Name`` field, type [systemitem]``Ubuntu 10 LTS AMD64 Main``.
* In the [guimenu]``Channel Label`` field, type [systemitem]``ubuntu-1804-amd64-main``.
* In the [guimenu]``Parent Channel`` field, select [systemitem]``ubuntu-10-pool for amd64``.
...
* In the [guimenu]``Channel Summary`` field, type [systemitem]``Ubuntu 10 LTS AMD64 Main``.
...
. Click btn:[Create Channel] and set these parameters for the entitlement certificate.
* In the [guimenu]``Channel Name`` field, type [systemitem]``Ubuntu 10 LTS AMD64 Main Updates``.
* In the [guimenu]``Channel Label`` field, type [systemitem]``ubuntu-1804-amd64-main-updates``.
* In the [guimenu]``Parent Channel`` field, select [systemitem]``ubuntu-10-pool for amd64``.
* In the [guimenu]``Architecture`` field, select [systemitem]``AMD64 Debian``.
* In the [guimenu]``Repository Checksum Type`` field, select [systemitem]``sha1``.
* In the [guimenu]``Channel Summary`` field, type [systemitem]``Ubuntu 10 LTS AMD64 Main Updates``.
* In the [guimenu]``Organization Sharing`` field, select [systemitem]``Public``.
. Click btn:[Create Channel].

. Navigate to menu:Software[Manage > Channels].
. Click btn:[Create Channel] and set these parameters for the entitlement certificate.
* In the [guimenu]``Channel Name`` field, type [systemitem]``Ubuntu 10 LTS AMD64 Main Security``.
* In the [guimenu]``Channel Label`` field, type [systemitem]``ubuntu-1804-amd64-main-security``.
* In the [guimenu]``Parent Channel`` field, select [systemitem]``ubuntu-10-pool for amd64``.
* In the [guimenu]``Architecture`` field, select [systemitem]``AMD64 Debian``.
* In the [guimenu]``Repository Checksum Type`` field, select [systemitem]``sha1``.
* In the [guimenu]``Channel Summary`` field, type [systemitem]``Ubuntu 10 LTS AMD64 Main Security``.
* In the [guimenu]``Organization Sharing`` field, select [systemitem]``Public``.
. Click btn:[Create Channel].

Your custom channels should use this structure, at menu:Software[Channel List > All] (example for {debian} 10:

* Base channel (vendor): [systemitem]``ubuntu-10-pool for amd64``
* Child custom channel: [systemitem]``Ubuntu 10 LTS AMD64 Main``
* Child custom channel: [systemitem]``Ubuntu 10 LTS AMD64 Main Updates``
* Child custom channel: [systemitem]``Ubuntu 10 LTS AMD64 Main Security``
* Child vendor channel: [systemitem]``Ubuntu-10-SUSE-Manager-Tools for amd64``
...
The root user on {debian} is disabled by default.
