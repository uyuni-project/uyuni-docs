...
Do not rely on them in a production environment.
====
...
The client must have NTP synchronized correctly with the {productname} server before registration.
...
* In the {productname} {webui}, navigate to menu:Software[Manage > Channels], and click on the channel associated to the repository.
