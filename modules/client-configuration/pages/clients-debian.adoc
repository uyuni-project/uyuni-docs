...
When synchronizing third-party {debian} repositories, you will need to import the appropriate GPG key.
If the GPG key is missing, synchronization will fail.
 
Only the metadata is signed, therefore importing a GPG key for the software channel is not needed.
