...
When synchronizing third-party {debian} repositories, you will need to import the appropriate GPG key - otherwise the sync will fail. Unlike Red Hat-based repositories, only the metadata is signed - so entering a GPG key url for the software channel is not needed.

To see which GPG keys {productname} currently has imported, run the following command:
