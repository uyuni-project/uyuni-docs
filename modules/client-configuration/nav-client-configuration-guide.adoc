...
// Contact Methods
** xref:contact-methods-intro.adoc[Contact Methods]
*** xref:contact-methods-salt.adoc[Salt Clients]
**** xref:contact-methods-saltssh.adoc[Push via Salt SSH]
// Add info on tunnel here
// The following can go away; it is covered in the previous chapter
