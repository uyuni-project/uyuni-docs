...
First create a [path]``setup_env.sh`` file in the [path]``/root`` directory defining the server setup like this:


. Here you define your certificate information and password.
The information are the same as <<proc-quickstart-certificate-information-yast>>.
+
----
CERT_O="SUSE"
CERT_OU="SUSE"
