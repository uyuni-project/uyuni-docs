...
. First describe your certificate information and password. The information are the same as <<proc-quickstart-certificate-information-yast>>.
+
----
CERT_O="SUSE"
CERT_OU="SUSE"
...
. Provide a description of your database. The user and password will be created during the setup. If you are using an external database, specify the hostname and port."
+
----
...
. Define the specific configuration for the external database.
...
. To connect to the external database, configure the report database with the RDS hostname and port. REPORT_DB_USER and REPORT_DB_PASS will be created by the script.
+
----
...
====
The certificate can be found at this address : https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/UsingWithRDS.SSL.html
