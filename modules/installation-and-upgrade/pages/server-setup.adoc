...
. First create a [path]``setup_env.sh`` file in the [path]``/root`` directory defining the server setup.
  The details are explained in the following steps.

. Set the variables defining your certificate information and password.
The information is the same as explained at <<proc-quickstart-certificate-information-yast>>.
+
...
The user and password will be created during the setup.
...
. Set the variables defining the specific configuration for the external database.
...
====
The certificate can be found at link:https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/UsingWithRDS.SSL.html[].
====
