...
* Running `registercloudguest --clean` can cause problems if no new cloud connection is registered with the cloud guest command.

* Refreshing or changing the [litera;]``registercloudguest`` connection to the public cloud update infrastructure should not interfere with the credentials usage.
====

== {payg} connector process

{payg} connector process uses:

* files stored in [path]``/etc/zypp/credentials`` 
* certificate that should alredy exist on the machine (instance?). This certificate is added by the cloud infrastructure.
* an entry in the machine's [literal]``hosts``file that is needed for the cloud plugin to work. It is automatically set by the installed cloud packages.

With this information available, a zypper plugin is invoked to get a valid authentication header.
(OM: I need a sentence here to bridge the {payg} connector and the authentication header.)

[IMPORTANT]
====
Any of the following actions/changes in the payg instance will lead to credentials stopping to work:
* Remove zypper credentials files
* Remove the imported certificates
* Remove cloud-specific entries from `/etc/hosts`
* Stop the service named "??"
* [literal]``systemd`` service (what is its name?), provided in cloud image, is running by default and should be keep running. If it is no longer running, the credentials may stop working.
====
