...
The goal of the connection is to extract authentication data so {productname} server is able to connect to cloud RMT host. That will give the {productname} server access to products that are on the RMT host and that are not already available via the {scc} organization credentials.
...
* valid {scc} credentials are entered in menu:Admin[Setup Wizard > Organization Credentials], even they provide only limited access to products - your subscription should at least offer access to the {productname} client tools.
...
[IMPORTANT]
====
If a product is provided by both the {payg} instance and by the {scc} subscription, it will appear only once in the products list.
