...
When the instance is not reachable, the credential update process will fail and the credentials will become invalid after the second failed refresh.
Synchronization of Channels will fail when the credentials are invalid. Please keep the connected instances running.
...
For more about troubleshooting {payg}, see xref:administration:troubleshooting/tshoot-payg.adoc[].
