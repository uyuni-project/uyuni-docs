...
. Move to the page section "Instance SSH conenction data".
...
[role=procedure]
. Go to the page section "Bastion SSH connection data".
...
. You will return to {payg} connection data Details page. The update connection status will be displayed on the top section named "Information".
. You can also see the connection status in [guimenu]``Admin > Setup Wizard > Pay-as-you-go`` screen.
