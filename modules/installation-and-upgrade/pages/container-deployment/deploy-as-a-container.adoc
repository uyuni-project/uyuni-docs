...
In this section you will learn how to prepare and deply an {productname} Server.
You will learn how to install [command]``Podman``, [filename]``Uyuni container utilities``, deployment and finally working with the container using [command]``mgrctl``.

This section assumes you have either a {tumbleweed} or {opensuse} {opensuse-version} server setup either on a physical machine or in a virtual machine.
...
For more information on [filename]``mgradm.yaml`` See, xref:installation-and-upgrade:container-deployment/mgradm-yaml-custom-configuration.adoc[Custom YAML configuration with mgradm]
