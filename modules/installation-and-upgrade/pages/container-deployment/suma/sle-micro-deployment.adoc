...
This process encompasses the installation of {micro}, and then initiating interaction with the container through the
...
Get a copy of the {micro} {microversion} installation media. You can begin the installation on either a virtual machine or on a physical server.
...
. Select your registration method. For this example we will register the sever with the {scc}.
...
Once installation completes, update the container host system.
...
. From the terminal of the container host, run the following command as the root user. Entering your servers FQDN is optional.
...
. Enter the administration account email address. Press btn:[Enter].
...
. Prepare a configuration file named [filename]``mgradm.yaml``mgradm.yaml`` similar to the following example:
+
...
. From the terminal run the following command as the sudo user. Entering your servers FQDN is optional.
+
