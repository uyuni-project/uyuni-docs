...
+
  ...
  The SSH key will be copy into the old server's [path]``~/.ssh/authorized_keys`` file.
...
. Establish an SSH connection from the new server to the old {productname} Server.
  So the fingerprint will be stored in the local [path]``~/.ssh/known_hosts`` file.
