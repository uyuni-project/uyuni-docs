...
* An in-place migration from {productname} 4.3 to {productnumber} is unsupported.
  The underlying operating system has been changed from {sles} 15 SP4 to {sle-micro} or {sles} {bci-suma}.
...
[IMPORTANT]
====
Do not pre-install {productname} on the prepared {sle-micro} {microversion} or {sles} {bci-suma} system. The migration process is designed to perform the server installation automatically.
