...
Then, add the SSH key to the running agent with [command]``ssh-add``; if the key is not in a standard file, provide it as an argument such as [command]``ssh-add </path/to/the/private/key>``.
