...
:description: Configure a new machine with SLE Micro 5.5 or SUSE Linux Enterprise Server 15 SP6, then install mgradm to migrate your SUSE Manager 4.3 server to a container
:revdate: 2025-07-03
...
* To migrate a {productname} 4.3 Server to a container, you require a new machine with {sle-micro} {microversion} or {sles} {bci-suma}, and [literal]``mgradm`` installed.

* An in-place migration from {productname} 4.3 to {productnumber} is not supported, regardless of whether the chosen host operating system is {sle-micro} {microversion} or {sles} {bci-suma}.
...
Migrating an existing {productname} {productnumber} instance to the same version while switching the host operating system from {sle-micro} {microversion} to {sles} {bci-suma}, or vice versa, is not handled by the [command]``mgradm migrate`` command.
====



////
// 2025-06-25, ke: with https://github.com/uyuni-project/uyuni-docs/pull/4056
//                 the Hostnames section is considered obsolete.  Thus
//                 commented for now.
=== Hostnames
...
[[prepare-micro-host]]
ifndef::backend-pdf[]
include::installation-and-upgrade:partial$prepare-micro-host.adoc[leveloffset=+2]
endif::[]

ifdef::backend-pdf[]
include::../../../partials/prepare-micro-host.adoc[leveloffset=+2]
endif::[]

[[deploy-sles-host]]
ifndef::backend-pdf[]
include::installation-and-upgrade:partial$prepare-sles-host.adoc[leveloffset=+2]
endif::[]

ifdef::backend-pdf[]
include::../../../partials/prepare-sles-host.adoc[leveloffset=+2]
endif::[]
