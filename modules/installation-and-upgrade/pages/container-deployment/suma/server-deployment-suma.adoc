...
Configuring persistent storage is optional, but the only way to avoid serious trouble with container full disk conditions.
It ist highly recommended to configure custom persistent storage with the [command]``mgr-storage-server`` tool.
...
==== Troubleshhoting Full Disk Event

<<<<<<< Updated upstream
    Increase the disk (e.g. /dev/vdb) size (e.g. if the disk is a QCOW2 image, use the command qemu-img resize)
    Ensure the Partition Uses All Available Space:
        open a shell in the container using mgrctl term
        sudo parted /dev/vdb
        (parted) print
        (parted) resizepart NUMBER 100% where NUMBER is the partition number shown by print command (e.g. 1 if /dev/vdb1
        (parted) quit
    resize the filesystem. e.g for a XFS file system run in a shell inside the container: sudo xfs_growfs /dev/vdb1

After these steps, your XFS filesystem on /dev/vdb1 should now be using all the available space on the disk.
=======
In case a dedicated disk that was mounted as a persistent storage medium runs out of storage space, proceed as follows to solve the problem.
...
. Resize the filesystem. For example, for a XFS filesystem run in a shell inside the container:
+
...
After these steps, a XFS filesystem on [literal]``/dev/vdb1`` should now be using all space available on the disk.
