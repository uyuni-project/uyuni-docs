...
[IMPORTANT]
====
SLE Micro is designed to update itself automatically by default and will reboot after applying updates. However, for {productname} environment, this behavior is not desirable. To prevent automatic updates on your server, we recommend disabling the [literal]``transactional-update`` timer.
...
This command will stop the timer from running immediately, ensuring that updates and reboots are managed manually on the server as needed.
====
