...
= {productname} {productnumber} Proxy Deployment on {sle-micro}
...
. In the [systemitem]``Product Description`` field select the {sle-micro} {microversion} checkbox then use the drop-down to select the [systemitem]``{productname} Proxy Extension 5.0 x86_64 BETA`` extension.
