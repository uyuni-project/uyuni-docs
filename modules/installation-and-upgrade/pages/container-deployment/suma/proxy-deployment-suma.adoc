...
// FIXME: ^^^ should we better just mention
//            /var/lib/containers/storage/volumes here above? ^^^
...
.{productname} {productnumber} Channel Synchronize for Proxy
...
. In the [systemitem]``Product Description`` field select the {sle-micro} {microversion} checkbox then use the drop-down to select the [systemitem]``{productname} Proxy Extension 5.0 x86_64`` extension.
