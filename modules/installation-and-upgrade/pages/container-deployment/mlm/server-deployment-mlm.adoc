...
For more information about importing SSL certificates, see xref:administration:ssl-certs-imported.adoc[].
====
...
. Execute one of the following commands, depending on the SSL certificate variant.
  Replace `<multi-linux-manager.example.com>` with your fully qualified domain name:
+

--
* With importing SSL certificates using third-party SSL certificate flags:
...
[IMPORTANT]
====
If one of the above commands fail ensure that you have registered {productname} {productnumber}.
