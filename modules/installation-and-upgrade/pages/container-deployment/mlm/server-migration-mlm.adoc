...
* To migrate a {productname} 4.3 Server to a container, you require a new machine with {sl-micro} {microversion} and [literal]``mgradm`` installed.

// FIXME?
* An in-place migration from {productname} 4.3 to {productnumber} is unsupported because of the change of the underlying operating system from {sles} 15 SP4 to {sl-micro} {microversion}.
...
Pre-installing {productname} on the prepared {sl-micro} {microversion} with [litaral]``mgradm`` is not necessary.
