...
[WARNING]
====
* The [command]``--prepare`` step (initial replication) can be run multiple times only as long as a final migration has not been started.

* Once the final migration is initiated and reaches the database upgrade stage, you must not run the [command]``migrate`` command again, as it can lead to data corruption.
====
