...
== Migrate SUSE Linux Enterprise Server 15 SP6 to 15 SP6
...
* System is registered with SUSE Customer Center (SCC) and has valid entitlements.
* Proxy system is registered with the {productname} Server.
...
If issues like GPG key errors occur:
...
Re-initiate the migration if needed.
...
[NOTE]
====
* If the reboot hangs or fails post-migration, consult system logs (`journalctl -xe`, `dmesg`) or verify snapshot rollback settings.
* Confirm that all mandatory channels were selected prior to migration.
====
