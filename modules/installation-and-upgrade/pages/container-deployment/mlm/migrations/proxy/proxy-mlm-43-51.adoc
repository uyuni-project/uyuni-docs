...
If you previously configured a 4.3 proxy to receive TFTP files, the following configuration steps are required:

run configure-tftpsync.sh in the container with the list of remaining 4.3 proxies as arguments. If no 4.3 proxies remain, run configure-tftpsync.sh with no arguments

OR

2. Manually remove the relevant proxy from the "proxies" setting in /etc/cobbler/settings.yaml in the container. If there are no 4.3 proxies remaining, then manually remove the "proxies" list completely.
