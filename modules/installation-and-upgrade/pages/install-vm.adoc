...
[IMPORTANT]
====
The following installation procedure is no longer recommended and will be replaced with the next documentation update.
For now it is recommended to install {productname} in a virtual machine environment using:

https://github.com/dvosburg/susemanager-contributions/blob/master/suma-from-vm-images/README.md
====
...
| OS:                 | Linux
| Version:            | SLES{sles-version}-{sp-base}-{minimal}-VM.x86_64-kvm-and-xen-GM.qcow2
| Memory:             | 16 GB
| CPU's:              | 4
| Storage Format:     | _.qcow2_ 24 GB (Default) Minimal Root Partition
| Virtual Disks:      |
