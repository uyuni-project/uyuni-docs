...
However, this command sets [literal]``/proc/sys/net/ipv6/conf/eth0/accept_ra = 1``.
As a result, the routes are no longer obtained via router advertisements and the default IPv6 route missing.

To enable correct functioning of the routing, follow the procedure:
