...
[IMPORTANT]
====
{productname} Server setup takes time. You can see the logs with [command]``journalctl -xeu uyuni-server.service``.
...
. In the destination server, add to `~/.ssh/config` :
+
```
Host host
...
If the source server user is not root,it should be able to run `rsync`. It can be done by adding to `/etc/sudoers`:
+
```
add to sudoers file
