= {productname} Server Upgrade
:revdate: 2025-07-29
:page-revdate: {revdate}

Before running the upgrade command, it is required to update the host operating system.
Updating the host operating system will also result in the update of the {productname} tooling such as the [literal]``mgradm`` tool.

.Procedure: Upgrading Server
[role=procedure]
____
. Refresh software repositories with [command]``zypper``:

+

[source,shell]
----
zypper ref
----

ifeval::[{mlm-content} == true]

. Depending on the host operating system, proceed with these steps:

+

For a transactional system such as {sl-micro}: ::

+

--

. Apply available updates with [command]``transactional-update``:

+

[source,shell]
----
transactional-update
----

. If updates were applied, [literal]``reboot``.

--

+

endif::[]

ifeval::[{uyuni-content} == true]

. Apply available updates:

+

[source,shell]
----
zypper dup
----

. If updates were applied, [literal]``reboot``.

endif::[]

ifeval::[{mlm-content} == true]

For {sles}: ::

+

--

Update installed software with [command]``zypper``:

[source,shell]
----
zypper up
----

--

endif::[]

+

. The {productname} Server container can be updated using the following command:

+

ifndef::backend-pdf[]
include::installation-and-upgrade:partial$snippet-ptf-loss-during-upgrade.adoc[]
endif::[]

ifdef::backend-pdf[]
include::../../partials/snippet-ptf-loss-during-upgrade.adoc[]
endif::[]

+

[source,shell]
----
mgradm upgrade podman
----

+

This command will bring the status of the container up-to-date and restart the server.

+

. Clean up the unused container images to free disk space:

+

[source,shell]
----
podman image prune -a
----

____

.Upgrading with third-party SSL certificate
[IMPORTANT]
====
If you are using third-party certificates, the database container needs to have an SSL certificate with the following Subject Alternate Names (SANs):

* [literal]``db``
* [literal]``reportdb``
* the externally facing fully qualified domain name

The same certificate can be used for both the main container and the database one, but it needs to have those SANs too.

In order to pass the new certificate to the upgrade command, use the [command]``--ssl-db-ca-root``, [command]``--ssl-db-cert`` and [command]``--ssl-db-key`` parameters.
====

.Upgrading to specific version
[NOTE]
====
If you do not specify the tag parameter, it will default to upgrading to the most recent version.
To upgrade to a specific version, provide the tag parameter with the desired image tag.
====


=== Avoid using `latest` tag for Uyuni containers

By default, Uyuni container images may use the `latest` tag:

----
registry.opensuse.org/uyuni/server:latest
registry.opensuse.org/uyuni/server-postgresql:latest
----

Using the `latest` tag is not recommended in production environments, as it can lead to unintended upgrades when new images are released.

==== Recommended approach

Use a fixed version when installing or upgrading containers via the tooling instead of relying on the default `latest` tag.

For example, specify the desired version when running the commands:

For stable Uyuni releases, use explicit version tags:

----
mgradm install podman --tag 2026.03
mgradm upgrade podman --tag 2026.03
----

For Uyuni snapshot repositories, images are stored in a separate path.
In this case, you need to explicitly specify the `--image` parameter:

----
mgradm install podman --image registry.opensuse.org/uyuni/server:latest
----

This approach also applies when using the proxy tooling:

For stable Uyuni releases:

----
mgrpxy install podman --tag 2026.03
mgrpxy upgrade podman --tag 2026.03
----

For snapshot repositories, proxy images are also stored in a separate path.
You need to explicitly specify the `--image` parameter:

----
mgrpxy install podman --image registry.opensuse.org/uyuni/proxy:latest
----

This ensures:

- predictable deployments
- reproducible environments
- controlled upgrade process

NOTE: In Uyuni, the main repository usually provides only the latest container image.
Older versions are available in the Snapshot repositories under a different path.

When using snapshots, ensure the correct image path is specified via the --image parameter.

To use a specific fixed version, ensure that the corresponding
Snapshots repositories are enabled; otherwise, the requested version may not be available.

Note that the default behavior may still use the `latest` tag if no version is specified.

For more information on the upgrade command and its parameters, use the following command:

ifndef::backend-pdf[]
include::installation-and-upgrade:partial$snippet-ptf-loss-during-upgrade.adoc[]
endif::[]

ifdef::backend-pdf[]
include::../../partials/snippet-ptf-loss-during-upgrade.adoc[]
endif::[]

[source,shell]
----
mgradm upgrade podman -h
----


For air-gapped installations, first upgrade the container RPM packages, then run the [literal]``mgradm`` command.



ifndef::backend-pdf[]
include::installation-and-upgrade:partial$snippet-database-backup-volume.adoc[leveloffset=+1]
endif::[]

ifdef::backend-pdf[]
include::../../partials/snippet-database-backup-volume.adoc[leveloffset=+1]
endif::[]
