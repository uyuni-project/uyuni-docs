...
Use a fixed version when upgrading containers via the tooling instead of relying on the default `latest` tag.

For example, specify the desired version when running the upgrade command:

----
mgradm upgrade podman --tag <version>
----
