...
However, it is recommended that the channels with the repositories and the database for {productname} are stored on separate storage devices.
...
The disk setup is virtual and separate disks for subvolumes are possible and recommended.

// container-deployment/suma/server-deployment-suma.adoc
For other deployments, separate disks for subvolumes are also recommended.

On the {productname} Server, the [command]``mgr-storage-server`` tool does it for the database and spacewalk volumes [literal]``var-pgsql`` and [literal]``var-spacewalk``.
On the {productname} Proxy, the [command]``mgr-storage-proxy`` tool does it for the cache volume [literal]``var-cache``.
