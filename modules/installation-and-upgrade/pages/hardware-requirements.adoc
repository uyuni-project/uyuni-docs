...
and optionally channels with the repositories and the database for {productname} are stored on separate storage devices.
...
ls -l /var/lib/containers/storage/volumes/var-spacewalk
----

The output should look like this:

// better use?  and change command to display dir, too?
// drwxr-x--- 1 postgres postgres 48 Apr 19 14:33 _data
// drwxr-x--- 1 wwwrun   www      30 Apr 19 14:34 _data
...
On the {productname} Server, the [command]``mgr-storage-server`` command moves the complete content of the [path]``volumes`` directory to a separate disk and remounts it to [path]``/var/lib/containers/storage/volumes``.
Optionally, if a second device name is specified, [command]``mgr-storage-server`` moves the content of the [literal]``var-pgsql`` database directory also to a separate disk and remounts it to [path]``/var/lib/containers/storage/volumes/var-pgsql``.

Similarily on the {productname} Proxy, the [command]``mgr-storage-proxy`` command moves the complete content of the [path]``volumes`` directory with the Squid cache to a separate disk and remounts it to [path]``/var/lib/containers/storage/volumes``.
