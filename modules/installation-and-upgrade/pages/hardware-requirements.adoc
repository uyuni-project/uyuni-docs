...
[WARNING]
====
Maximum number of clients which can be managed by a single {productname} server is 10 000.
Registering higher than maximum number is not supported by {suse}.
Customers can ask support for advice or exception depending on their own workload.
