...
Otherwise we either write {sle-micro} or just {hostos}.
...
On the container host, use this command to find all available storage devices:
...
Similarily, on the container host of the {productname} Proxy, the [command]``mgr-storage-proxy`` command moves the complete content of the [path]``/var/lib/containers/storage/volumes`` directory to a separate disk and remounts it to [path]``/var/lib/containers/storage/volumes``.
