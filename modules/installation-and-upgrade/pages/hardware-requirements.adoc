...
For all kind of virtual machines (VM) you do not need LVM because the disk is virtual and thus can be increased.
With containers {productname} now has [path]``/var/lib/containers/storage/volumes`` as an extra disk.
Additional disks for subvolumes are possible, and [command]``mgr-storage-server`` does it for the database volume [literal]``var-pgsql``.

When an installation takes place on bare metal (on-premise}, [command]``cockpit`` can be used to create an LVM setup.
