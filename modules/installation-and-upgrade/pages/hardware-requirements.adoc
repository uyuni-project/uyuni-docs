...
total 0
drwxr-xr-x 1 wwwrun   www      30 Apr 19 14:34 _data
----
...
Optionally, if a second device name is specified, [command]``mgr-storage-server`` moves the content of the [literal]``/var/lib/containers/storage/volumes/var-pgsql`` database directory also to a second separate disk and remounts it to [path]``/var/lib/containers/storage/volumes/var-pgsql``.

Similarily on the {productname} Proxy, the [command]``mgr-storage-proxy`` command moves the complete content of the [path]``/var/lib/containers/storage/volumes`` directory with the Squid cache to a separate disk and remounts it to [path]``/var/lib/containers/storage/volumes``.
