...
.Procedure: Hub server

. Install a container host with {sl-micro}.
For more information, see xref:installation-and-upgrade:container-deployment/suma/server-deployment-suma#deploy-suma-server-micro[].
...
* Next, on every peripheral server host, deploy {productname} with:
+
