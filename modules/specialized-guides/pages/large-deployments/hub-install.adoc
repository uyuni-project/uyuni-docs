[[lsd-hub-install]]
= Hub Installation
:description: Configure the hub-xmlrpc-api package on the Hub Server or a host with access to all peripheral servers XMLRPC APIs
:revdate: 2025-06-06
...
Do this by copying the ``RHN-ORG-TRUSTED-SSL-CERT`` certificate file from each peripheral Server's **+http://<server-url>/pub/+** directory to [path]``/etc/pki/trust/anchors/``, and run [command]``update-ca-certificates``.
====
