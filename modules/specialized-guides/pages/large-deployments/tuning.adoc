...
| Description          | The number of Apache httpd processes serving HTTP requests simultaneously.
                         The number must equal xref:max-clients[`MaxRequestWorkers`].
| Tune when            | xref:max-clients[`MaxRequestWorkers`] changes
| Value default        | 150
| Value recommendation | The same value as xref:max-clients[`MaxRequestWorkers`]
| Location             | [path]``/etc/apache2/server-tuning.conf``, in the `prefork.c` section
...
| Tune when            | xref:max-clients[`MaxRequestWorkers`] changes.
                         `maxThreads` must always be equal or greater than xref:max-clients[`MaxRequestWorkers`]
| Value default        | 150
| Value recommendation | The same value as xref:max-clients[`MaxRequestWorkers`]
