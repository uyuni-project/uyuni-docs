...
[IMPORTANT]
====
The dropdown only exists on custom channels which do not exist yet on the peripheral server.
If the channel exists, the organization will stay unchanges.
====
...
With this, the channels will be created on the peripheral and everything will be setup to mirror the channels on the next regular repository synchronization task.

+
[IMPORTANT]
====
The repository synchronization will not start directly.
====
