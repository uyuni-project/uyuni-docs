...
.Peripheral Servers List Ccolumns
|===
| Column                      | Description
...
. Click on the fully qualified domain name of the peripheral server to access its details.
...
.. Field [literal]``Root Certificate Authority`` shows certificate details.
   ...
   Deleting the certificate will break the connection between servers.
.. Field [literal]``Mirror credentials`` is the username the peripheral server uses when connecting to the hub server to synchronize vendor channels.
   This username is generated automatically on the hub server, and then transmitted to the peripheral server during the registration phase. To generate new credentials, click on btn:[Regenerate Credentials]. This action will create a new password and transmit it 
   securely to the peripheral server.
