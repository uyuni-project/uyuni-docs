...
[IMPORTANT]
====
In any case, if the peripheral host should be managed by the hub, first bootstrap the host as a minion to the hub before proceeding
with the registration of the peripheral server. If the bootstrapping happens later, 2 systems are shown in the systems list.
====
...
The repository synchronization can be started from the peripheral server

.Procedure: Trigger repository synchronization from the peripheral server
