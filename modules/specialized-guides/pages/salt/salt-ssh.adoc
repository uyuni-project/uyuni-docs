...
{productname} keeps these details in the database and makes them available to Salt by using `uyuni` roster module or by generating a temporary roster file on bootstrapping new clients with {webui} using Salt SSH call.
The location of the temporary roster file is supplied to `salt-ssh` using the `--roster-file=` option.
For registered clients `--roster=uyuni` is used instead.

[IMPORTANT]
====
It is not recommended to use [command]``salt-ssh`` as the call will be processed with current user permissions and in case of calling it with `root` user can cause permission issues for the files on processing next calls with `salt-api`.
Use [command]``mgr-salt-ssh`` instead because it is changing effective user to `salt` on each call.
If you want to use it with a user other than `root`, the user should have the permission to change effective user to `salt`.
====
...
* `ssh_pre_flight`: The path to the pre flight shell script
