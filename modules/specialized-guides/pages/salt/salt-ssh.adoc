[[salt.ssh]]
= Salt SSH
:description: Configure Salt SSH connections in SUSE Manager to execute commands and manage states directly over SSH
:revdate: 2025-06-06
...
Traditionally managed systems with SSH push are not affected and the key cannot be rotated by this mechanism.
====
