...
When the {productname} is installed using `mgradm install` or `mgrpxy install`, it sets up Podman which enables IPv4 and IPv6 forwarding.
...
| 69          | TCP/UDP  | TFTP       | Required if server is used as a PXE server for automated client installation (starting with version 5.2, TFTP is disabled on the server by default and is recommended to run on the proxy).
...
| 67          | TCP/UDP  | DHCP    | Required only if clients are requesting IP addresses from the server.
| 69          | TCP/UDP  | TFTP    | Required if the proxy is used as a PXE server for automated client installation (recommended).
...
In order to perform confidential computing attestation for IBM, the system needs to be to be able to autonomously download the certificates and the certificate-revocation lists.
Therefore the following addresses must be reachable, even behind a HTTP proxy:
