...
:description: Install Proxy in an air-gapped environment using the provided Virtual Machine Image or SL Micro with pre-loaded container images and upgrade with mgrpxy upgrade
:revdate: 2026-04-29
...
To upgrade {productname} Proxy, upgrade all packages on the host system, then run `mgrpxy upgrade`.
For air-gapped installations, upgrade the container RPM packages before running `mgrpxy upgrade`.
