...
:description: Deploy Server in an air-gapped environment by pulling container images from a connected system and transferring them to the Server host
:revdate: 2025-07-25
...
To upgrade {productname} Server, apply available host updates with `transactional-update`, reboot if updates were applied, then run `mgradm upgrade`.
For air-gapped installations, upgrade the container RPM packages before running `mgradm upgrade`.
