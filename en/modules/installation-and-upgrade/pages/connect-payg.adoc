...
:description: Connect PAYG instance to a server to access additional products by entering the instance's SSH connection details and authentication information so the server
:revdate: 2026-09-07
...
[IMPORTANT]
====
Deleting a {payg} connection configuration or its SSH connection data has the following effects on your server:

* Already synchronized software channels and packages remain on the server database and package storage. Deleting the connection does not automatically remove any existing data.
* Since deleting the connection removes the harvested public cloud credentials, all future synchronizations (`reposync`) of those channels will fail (typically with an HTTP 401 Unauthorized or credential expiration error).
* To resume channel synchronization, you must re-establish a valid {payg} connection to retrieve fresh credentials.
====
