[[retail-delta-images]]
= Delta Images
:description: Configure delta images using the retailcreatedelta tool to minimize network bandwidth consumption during synchronization
:revdate: 2024-06-05
...
The `retail_create_delta` tool creates a delta image on the {productname} server.
The tool uses `xdelta3` internally.
...
The resulting delta file is saved in `/srv/www/os-images` and the corresponding pillar in the database.



== Tuning Delta Generation

Performance tuning is possible with the `-B <VALUE>` option, which is passed to `xdelta3`.
With higher values you achieve smaller deltas at the cost of higher memory requirements.
For more information, see the `xdelta3` documentation (`man xdelta3`).
