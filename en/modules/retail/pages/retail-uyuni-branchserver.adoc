...
:description: Install and configure the Retail Branch Server by adding software channels, checking synchronization status, trusting GPG keys on clients, and creating activation.
:revdate: 2025-06-24
...
----
+
  `configure-proxy.sh` is an interactive script.

. OPTIONAL: If you want to use the same system also as a build host, navigate to the client's system profile and check `OS Image Build Host` as a `Add-On System Types`.
...
----
+
  For additional options,  use the `retail_branch_init --help` command.
