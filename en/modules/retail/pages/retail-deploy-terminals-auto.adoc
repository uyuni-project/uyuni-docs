...
[role=procedure]
____
. In the location where the image source is built, such as a build host or source repository, create a configuration file called `etc/ventv-salt-minion/minion.d/autosign-grains.conf`.
...
. PXE boot the {salt} client.
  The formula creates the `./etc/salt/minion.d/autosign-grains-onetime.conf` configuration file and passes it to `initrd`.
____
