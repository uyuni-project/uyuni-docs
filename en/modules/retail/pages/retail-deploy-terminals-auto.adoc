...
:description: Configure the server to automatically accept keys from newly deployed terminals by creating an auto-sign file that contains the key used during Saltboot setup
:revdate: 2026-04-30
...
The server stores the autosign keys in a file within the `/etc/salt/master.d/` directory, which is preserved in `etc-salt` volume.
...
--
.. In directory `/etc/salt/master.d/`, create file `autosign_grains.conf` and specify `autosign_grains_dir`:
...
.. Create a file at `/etc/salt/autosign_grains/autosign_key`, that contains the auto-sign key you specified with {saltboot} (replace `<AUTOSIGN_KEY>` with the key:
...
[role=procedure]
____
. In the location where the image source is built, such as a build host or source repository, create a configuration file called `etc/venv-salt-minion/minion.d/autosign-grains.conf`.
...
____
. In the location where the image source is built, such as a build host or source repository, create a configuration file called `etc/venv-salt-minion/minion.d/autosign-grains.conf`.
  This must be outside of the `root` directory provided by the template.
...
. Edit the `config.xml` template file.
  In the `<packages type="image">` element, add:
...
. PXE boot the {salt} client.
  The formula creates the `./etc/venv-salt-minion/minion.d/autosign-grains-onetime.conf` configuration file and passes it to `initrd`.
____
