...
:description: Set up the Retail environment by installing required components, preparing POS images, and configuring system groups and network services for Saltboot
:revdate: 2025-01-21
...
Containerized {productname} {smr} is configured using so called `Saltboot Group`.
...
`Saltboot Group` formula is a successor of `Branch Network formula`, `PXE formula` and `TFTP formula` previously used in {productname} {smr} setups.
...
For this to work `DHCP` service for the network terminal is connected to must have `PXE` or sometimes called `BOOTP` support enabled.
...
[IMPORTANT]
====
{productname} {smr} branch proxy uses different `filename` then previous non-containerized branch server.
