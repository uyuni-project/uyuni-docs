...
= Deployment basics
:description: Learn how to deploy point-of-service terminals by creating hardware type groups, assigning Saltboot formulas, and synchronizing images in the SUSE Multi-Linux
:revdate: 2024-06-05
...
. Determine the hardware type group name.
  Prefix the name with `HWTYPE:`, followed by the system manufacturer name and product name, separated by a hyphen.
  For example:
+
----
...
. In the `Create System Group` dialog, create a new system group, using the hardware type group name you determined in step one of this procedure.
...
Manual synchronization of images is no longer needed with {susemgrbranch}. It is however possible to warm up the cache if required.

Squid cache on the {susemgrbranch} stores OS images with extremely long freshness and are not usually purged from the cache.
