...
Where
  ...
  * [literal]``$branchproxy`` is a branch server fully-qualified domain name
  * [literal]``$url`` is the path to the image file shown in the WebUI image details page
  * [literal]``$orgid`` is the numerical identified of the organization owning this branch server instance
