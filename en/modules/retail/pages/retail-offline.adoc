[[retail.offline]]
= Offline Use
:description: Use the service partition to cache POS images on the terminal when the Server is offline so that updates can be performed without a stable connection between the Server and the terminal
:revdate: 2023-07-27
...
You can adjust the timeout value by changing the `SALT_TIMEOUT` kernel parameter.
The parameter is measured in seconds, and defaults to `60`.
