...
:description: Install Retail Server with openSUSE Leap by adding the Server software pattern to an updated openSUSE Leap system and rebooting
:revdate: 2025-06-25
...
----
. Add the repository for installing the {productname} Server software as `root`:
+
----
zypper ar https://download.opensuse.org/$repo uyuni-server-stable
----
. Refresh metadata from the repositories as `root`, and confirm the import of new GPG key into the keyring:
+
----
zypper ref
----
. Install the pattern for the {productname} Server as `root`:
+
----
zypper in patterns-uyuni_server
----
. Install the pattern for the {productname} {smr} product as `root`:
+
----
