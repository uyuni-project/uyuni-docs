[[retail.mass.config]]
= Mass Configuration
:description: Configure and deploy pre-defined configurations for multiple branch servers or terminals simultaneously using SUSE Multi-Linux Manager for Retail by creating
:revdate: 2025-02-06
...
Configuring multiple branch servers requires the `python-susemanager-retail` package, which is provided with {productname} {smr}.
Install the `python-susemanager-retail` package on the {productname} server.
...
. In the `States` tab, click btn:[Apply Highstate] to deploy your configuration changes for each branch server.
