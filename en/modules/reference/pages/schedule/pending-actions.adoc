...
= Pending Actions
:description: Find and manage actions that are waiting to be completed or are currently running in the Pending Actions section by using the filter options and selecting items
:revdate: 2020-09-11
...
If you archive a pending action, it is not canceled, but the action item moves from the `Pending Actions` list to the `Archived Actions` list.
...
Click the action to go to `Action Details`.
