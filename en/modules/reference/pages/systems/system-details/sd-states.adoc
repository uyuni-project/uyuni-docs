...
= States
:description: Manage Salt states on the selected client using the Systems[States] menu section to apply configuration templates and control package and configuration
:revdate: 2023-05-08
...
To set the package state to install the package, select `Installed` as the new state.
To always upgrade the package to the latest version, select `latest`.
