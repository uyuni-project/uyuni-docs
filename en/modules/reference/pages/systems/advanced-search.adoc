[[ref-systems-search]]
= Advanced Search
:description: Use the Advanced Search section to look for systems, network details, hardware, and other information across your environment by entering search terms
:revdate: 2020-09-11
...
Type your search term in the `Search For` field.
Select the criterion to search by, use the radio buttons to specify whether you want to query all systems or only those in the `System Set Manager`, and click the btn:[Search] button.
Check `Invert Result` to list only results that do not match the specified criteria.
