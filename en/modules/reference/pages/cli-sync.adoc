...
:description: Use the Synchronization CLI Tools to manage client synchronization with the server by selecting between mgr-sync for clients connected to the SUSE Customer
:revdate: 2024-11-29
...
The `spacewalk-repo-sync` tool synchronizes software repositories into {productname} channels.
...
. Log the HTTP output into `/var/log/zypper.log`:
+
----
...
----
  You can increase the debug level, by adding the `-vvv` option to the command.
