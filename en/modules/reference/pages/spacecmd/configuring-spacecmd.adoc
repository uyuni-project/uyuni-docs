...
. Edit the `config` file and add the following configuration lines. (You can use either localhost, when running spacecmd from your {productname} server container, or the FQDN of your {productname} server):
+
[source]
----
[spacecmd]
...
spacecmd help can be accessed by typing spacecmd `-h --help`
