[[ref-admin-wizard]]
= Setup Wizard
:description: Configure the SUSE Multi-Linux Manager settings to include HTTP proxy connections and organization credentials for access
:revdate: 2025-01-20
...
Navigate to the `HTTP Proxy` tab to configure the proxy.
...
Navigate to the `Organization Credentials` tab to grant users within your organization access to your {scclongform} account.
...
Navigate to the `Products` tab to browse the products available to you and synchronize {productname} with {scclongform}.
