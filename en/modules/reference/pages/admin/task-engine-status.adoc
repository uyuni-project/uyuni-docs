[[ref-admin-status]]
= Task Engine Status
:description: View the Task Engine Status page to see information about all running tasks, including their last execution times, current status, and task details.
:revdate: 2020-09-11
:page-revdate: {revdate}

The menu:Admin[Task Engine Status] section shows all running tasks by the {productname} task engine.

Navigate to the `Last Execution Times` tab to see the task list.
Each task shows the time it was last run, and the current status of the task.

Navigate to the `Runtime Status` tab to see all tasks that have run in the past five minutes.
Each task shows the start and end time, the amount of time the task ran for, and the current status.
Some tasks will also provide further data, if available.
