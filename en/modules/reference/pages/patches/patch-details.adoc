[[ref-patches-details]]
= Patch Details
:description: View patch details to see information about a selected patch including advisory numbers, affected channels, security vulnerability details, and vendor advisories
:revdate: 2021-11-02
:page-revdate: {revdate}

The menu:Patches[Patch List > Patch Details] section displays the details of a selected patch.
Click the advisory number of a patch in the `Patch List` to see more information about the patch.
...
In the `Affected Channels` section, all channels that contain the affected package are listed.
...
The `Packages` tab provides links to each of the updated packages by channel.
...
The `Affected Systems` tab provides a list of installed clients that the patch affects.
...
Click the name of a status in the list to go to the `Action Details` page.
