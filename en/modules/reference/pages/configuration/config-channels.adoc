[[ref-config-channels]]
= Channels
:description: Manage configuration files using Config Channels and State Channels to store, edit, and deploy settings to subscribed systems
:revdate: 2025-01-20
:page-revdate: {revdate}

{productname} manages configuration files through two channel types: `Config Channels` and `State Channels`.
Config Channels store managed configuration files.
State Channels use Salt states and provide an editable `init.sls` file.
...
If you click the number of files in the channel, you are taken to the `List/Remove Files` page of that channel.
...
Overview::
The `Overview` page of the `Configuration Channel Details`
...
For Salt clients, there is a link to edit the `init.sls` file.
+
...
You can remove files or copy the latest versions into other configuration channels.
...
+
Import Files:::
To import files from other configuration channels, check the box to the left of any file you want to import.
...
Click the name of a system to see the `System Details` page.
