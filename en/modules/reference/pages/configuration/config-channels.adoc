...
{productname} manages configuration files through two channel types: [guimenu]``Config Channels`` and [guimenu]``State Channels``.
Config Channels store managed configuration files.
State Channels use Salt states and provide an editable [path]``init.sls`` file.
