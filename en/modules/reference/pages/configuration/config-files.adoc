[[ref-config-files]]
= Files
:description: Learn how to manage configuration files independently and configure files up to 1 MB on SUSE systems by editing variables in the /etc/rhn/rhn.conf file
:revdate: 2026-04-20
...
|===
Click the `Path` of a file to see its `Details` tab.
Click the name of the `Configuration Channel` to see the `Overview` tab of the channel.
Clicking `Systems Subscribed` shows all systems currently subscribed to the channel containing that file.
...
Copy these variables to `/etc/rhn/rhn.conf` and edit them.
