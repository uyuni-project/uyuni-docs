...
:description: Configure your SUSE environment with a central hub server; connect multiple peripheral servers to synchronize channels, repositories, and packages efficiently
:revdate: 2026-07-14
...
Hub online synchronization is the primary mechanism for connecting peripheral servers to the hub.
It establishes the underlying connection required for other hub features: xref:specialized-guides:large-deployments/hub-api.adoc[Hub XMLRPC API] and xref:specialized-guides:large-deployments/hub-reporting.adoc[Hub Reporting].
...
. Click button btn:[Add token] and select option `Issue new token`.
. In the field `Server FQDN` on the form that opens type the FQDN of the hub server that will be using this token.
. Click btn:[Issue].
...
. Once generated, the token appears on the screen `Access Tokens`.
_____
...
. Click button btn:[Add peripheral].
  A new form `Register a new peripheral server` opens.
. In the field `Peripheral Server FQDN` enter the name of the peripheral server.
. In the field `Registration mode` select option `Existing token`.
. In the field `Token` paste the token that was created on the peripheral server.
. In the field `Root CA certificate` specify the certificate using one of the options:
...
. Click button btn:[Register].
  A newly registered peripheral server will appear on screen `Peripherals Configuration`.
_____
...
. Click button btn:[Add peripheral].
  A new form `Register a new peripheral server` opens.
. In the field `Peripheral Server FQDN` enter the name of the peripheral server.
. In the field `Registration mode` select option `Administrator User/Password`.
. In the fields `Username` and `Password` enter the credentials for the peripheral server. 
...
. Click button btn:[Register].
. The newly registered peripheral server will be shown in the menu:Systems[System List] with the value `Foreign` in the column `System Type`.
...
=== Migrating from previous ISS versions

If you have peripheral servers that were previously configured using legacy Inter-Server Synchronization versions (ISS v1 or ISS v2), you can migrate them to Hub Online Synchronization (ISS v3) directly from the WebUI.

[IMPORTANT]
====
The {productname} Hub reporting database and Hub XML-RPC API rely entirely on a valid and active Hub Online Synchronization (ISS v3) registration to function. After any Hub upgrade or migration, ensure that all peripheral servers are successfully migrated or registered under ISS v3.
====

==== Migrating from ISS v1

.Procedure: Migrating from ISS v1
[role=procedure]
_____
. On the Hub server, go to menu:Admin[Hub Configuration > Peripherals Configuration].
. Click the btn:[Migrate] dropdown button and select option `Migrate ISS v1...`.
. The page displays a table of detected ISS v1 peripheral servers. Select the servers you want to migrate.
. In the `Token` field for each selected server, enter the access token generated on that peripheral server.
. Specify the root CA certificate using one of the options (for example, select `Not needed` if both servers share the same CA).
. Click btn:[Migrate]. Once successfully completed, the migrated servers will appear in the `Peripherals Configuration` list.
_____

==== Migrating from ISS v2

Since ISS v2 is a manual, file-based mechanism, you must specify the peripheral server details manually during migration.

.Procedure: Migrating from ISS v2
[role=procedure]
_____
. On the Hub server, go to menu:Admin[Hub Configuration > Peripherals Configuration].
. Click the btn:[Migrate] dropdown button and select option `Migrate ISS v2...`.
. Click btn:[Add Server].
. In the modal dialog that opens, enter the `Server FQDN` of the remote peripheral server and click btn:[Save].
. In the `Token` field, enter the access token generated on that peripheral server.
. Specify the root CA certificate using one of the options (for example, upload or paste the PEM certificate).
. Click btn:[Migrate]. Once successfully completed, the migrated servers will appear in the `Peripherals Configuration` list.
_____
...
Consumed::
The `Consumed`token is generated on the peripheral server and received by the hub server to be used.
...
Consumed::
The `Consumed`token is generated on the hub server and received by the peripheral server to be used.
...
. On the screen `Hub Details` find the information about the hub server.
.. Field `Hub server FQDN` shows the hub server's FQDN.
.. Field `Registration date` shows the time when the peripheral server was registered to the hub server. 
.. Field `Last modified` shows the time of the last saved configuration change. 
.. Field `Root Certificate Authority` shows certificate details.
...
.. Field `GPG Public Key` shows whether the GPG key has been configured for the hub server.
   For more information about GPG keys between hub and peripheral servers, see <<gpg-for-hub-online-sync>>.
.. Field `Mirror credentials` is the username the peripheral server uses when connecting to the hub server to synchronize vendor channels.
...
When the GPG key is created on the hub, field `GPG Public Key` will be set to show that this server is using the GPG key.
...
. On the screen `Peripheral Details` .
.. Field `Peripheral server FQDN` shows the peripheral server's FQDN.
.. Field `Registration date` shows the time when the peripheral server was registered to the hub server. 
.. Field `Last modified` shows the time of the last saved configuration change. 
.. Field `Root Certificate Authority` shows certificate details.
...
.. Field `Synchronized channels` shows the number of currently synchronized channels and organization. 
_____ 
...
. Find the perpheral server on the list
. Click the fully qualified domain name of the peripheral server to access its details.
. In the field `Synchronized channels` click on btn:[Edit channels].
. Page `Sync Channels from Hub to Peripheral` opens.
