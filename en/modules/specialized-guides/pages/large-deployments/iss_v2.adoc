...
:description: Copy data between hub and peripheral servers using Inter-Server Synchronization version 2 by exporting content from the hub server and importing it
:revdate: 2025-06-04
...
.. Execute the ISS export command.
   The `-h` option provides detailed help:
...
[IMPORTANT]
====
To avoid data loss or filling up the container file system, store the data on a persistent volume (preferably `/var/spacewalk`).
