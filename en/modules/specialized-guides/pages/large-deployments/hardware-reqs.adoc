...
:description: Find the minimum hardware requirements for the Server and Proxy and learn how to configure storage schedulers and estimate client download times
:revdate: 2026-08-12
...
[IMPORTANT]
====
All hardware recommendations are **guidelines only**.
The actual hardware needs will be influenced by number of clients, channels operations, and other parameters.

For succesful configuration of large deployments, seek guidance from SUSE Support Team.
====
...
| CPU
| {x86_64}
| Minimum 8 dedicated 64-bit CPU cores
...
You can check the current status with `cat /sys/block/<DEVICE>/queue/scheduler`.
This command will display a list of available schedulers with the currently active one in brackets.
To change the scheduler before a reboot, use `echo none > /sys/block/<DEVICE>/queue/scheduler`.

The minimum requirements for the {productname} Proxy are:


[cols="1,2,3", options="header"]
.Minimum Proxy Hardware Requirements
|===
...
|
| 
| Additionally, include sufficient storage for caching.

| Number of clients
| Recommendation
| One proxy is generally reasonable for **500-1000 clients**, but this is a **recommendation, not a hard limit**, and depends on available network bandwidth
