...
:description: Learn how to optimize SUSE Multi-Linux Manager performance by following the steps outlined in this document for adequate hardware sizing and parameter tuning
:revdate: 2026-07-01
...
== Upload files larger then 1GB

.Procedure: Uploading files larger then 1GB
[role=procedure]
____

. On the server, step inside the container environment:
...
. Create new file `/etc/apache2/conf.d/maxrequestbody.conf` to make request size unlimited:
