...
:description: Tune your system by adjusting configuration parameters based on environmental variables and using the dependency graph to improve performance
:revdate: 2026-08-06
...
| Tune when            | xref:user-count[User count] and proxy count increase significantly and this line appears in `/var/log/apache2/error_log`: `[...] [mpm_prefork:error] [pid ...] AH00161: server reached MaxRequestWorkers setting, consider raising the MaxRequestWorkers setting`.
| Value default        | 150
...
| Tune when            | The number of queued Salt events grows. Typically, this can happen during onboarding of large number of minions with higher value of
                         ...
                         The number of events can be queried by `echo "select count(*) from susesaltevent;" \| spacewalk-sql --select-mode-direct - `
| Value default        | 8
...
| Tune when            | xref:client-count[Client count] increases significantly, or some clients are responding correctly but too slowly, and {productname} excludes them from calls.
                         This line appears in `/var/log/rhn/rhn_web_ui.log`: `"Got no result for <COMMAND> on minion <MINION_ID> (minion did not respond in time)"`
...
[IMPORTANT]
====
In containerized {productname} deployments, insufficient Taskomatic worker threads can cause scheduled system tasks to remain permanently in “pending” state.
If you increase this parameter to clear pending tasks, verify that your container environment has adequate CPU limits to handle the additional concurrent task processing. 
Monitor `/var/log/rhn/rhn_taskomatic_daemon.log` to verify if worker threads are exhausted.
====
...
[IMPORTANT]
====
When increasing Taskomatic's thread count or memory allocation to resolve stuck pending tasks in a containerized deployment, you must verify that the underlying container's memory limit is set higher than the `taskomatic.java.maxmemory` value. 
If the container does not have sufficient hardware capacity or limits are too restrictive, task processing will fail or the container will be unexpectedly terminated by the host.
====
...
| Location             | `/var/lib/containers/storage/volumes/var-pgsql/_data/postgresql.conf`
...
To test if that is happening, add `log_temp_files = 5120` to `/var/lib/containers/storage/volumes/var-pgsql/_data/postgresql.conf`, restart PostgreSQL, and monitor the PostgreSQL log files.
...
=== `DISKCHECKALERT` and `DISKTHRESHOLD`

[cols="1,1"]
|===
| Description          | Environment variables that control disk usage monitoring thresholds for containerized deployments.
                         `DISKCHECKALERT` triggers a warning notification when disk usage exceeds the specified percentage.
                         `DISKTHRESHOLD` triggers a critical notification and causes the container health check to fail, which will automatically stop the container.
                         In large deployments with multi-terabyte storage volumes, the default thresholds (90% and 95%) can leave excessive amounts of disk space unused.
                         For example, on a 10{nbsp}TB volume, 5% free space equals 500{nbsp}GB, which is wasteful.
                         Increasing these thresholds allows more efficient use of available storage capacity while maintaining adequate headroom for operations.
| Tune when            | Large storage volumes are deployed (multiple terabytes) and you want to use disk space more efficiently while maintaining adequate headroom for database growth and repository synchronization.
| Value default        | `DISKCHECKALERT=90`, `DISKTHRESHOLD=95`
| Value recommendation | For deployments with 5{nbsp}TB+ storage: `DISKCHECKALERT=95`, `DISKTHRESHOLD=98`.
                         For deployments with 10{nbsp}TB+ storage: `DISKCHECKALERT=96`, `DISKTHRESHOLD=99`.
| Location             | Systemd service override files: `/etc/systemd/system/uyuni-server.service.d/diskcheck.conf` and `/etc/systemd/system/uyuni-db.service.d/diskcheck.conf`
| Example              | See the procedure below
| After changing       | Monitor disk usage notifications and ensure adequate space remains for database growth, package repository synchronization, and temporary operations.
                         Use `df -h` to verify current disk usage levels.
| Notes                | The disk check runs hourly.
                         Both the server and database containers should use the same threshold values.
                         When `DISKTHRESHOLD` is exceeded, container health checks will fail and the container will be automatically stopped.
                         The database container only monitors `/var/lib/pgsql/data` and this cannot be overridden with `DISKCHECKDIRS`.
| More information     | xref:administration:space-management.adoc[]
|===


.Procedure: Configuring Disk Check Thresholds for Large Deployments

. Create systemd override directories for both services:
+
----
...
. Create configuration files with higher thresholds for the server service:
+
----
...
. Create configuration files with higher thresholds for the database service:
+
----
...
Operating system::
Stop all {productname} services and inspect the output of `free -h`.
