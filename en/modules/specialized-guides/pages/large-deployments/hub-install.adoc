[[lsd-hub-install]]
= Hub deployment
:description: Deploy a hub environment using third-party or self-generated certificates and install the hub server and peripheral servers
:revdate: 2026-05-21
:page-revdate: {revdate}

[NOTE]
====
The connection between hub and peripheral servers is now primarily established using xref:specialized-guides:large-deployments/hub-online-sync.adoc[Hub Online Synchronization].
This chapter describes an optional but recommended best practice to use the same CA for all servers.
====
...
// Hub:
// mgradm install --ssl-ca-root CA-Certificate.crt --ssl-server-cert hub.crt --ssl-server-key hub.key --hubxmlrpc-replicas 1

// Peripheral servers:
// mgradm install --ssl-ca-root CA-Certificate.crt --ssl-server-cert server.crt --ssl-server-key server.key
...
. On the container host, deploy {productname} as the hub server using the third party certificate.
  Add `--hubxmlrpc-replicas 1` to the `mgradm install` command line.
  For example:

+

----
mgradm install --ssl-ca-root CA-Certificate.crt --ssl-server-cert hub.crt --ssl-server-key hub.key --ssl-db-ca-root CA-Certificate.crt --ssl-db-cert hud-db.crt --ssl-db-key hub-db.key --hubxmlrpc-replicas 1
----

_____

For more information about deploying with `mgradm`, see xref:installation-and-upgrade:container-deployment/mlm/server-deployment-mlm.adoc#deploy-mlm-server-mgradm[].
...
. Preliminary Requirement: A certificate for every peripheral server and its database (for example, `server.crt`) and a key (for example, `server.key`).
...
----
mgradm install --ssl-ca-root CA-Certificate.crt --ssl-server-cert server.crt --ssl-server-key server.key --ssl-db-ca-root CA-Certificate.crt --ssl-db-cert db.crt --ssl-db-key db.key
----
...
----
mgradm install MLM.example.com --hubxmlrpc-replicas 1
----
...
. Inside the container, run `rhn-ssl-tool` for every peripheral server:
...
----
mgradm install --ssl-ca-root RHN-ORG-TRUSTED-SSL-CERT --ssl-server-cert server.crt --ssl-server-key server.key --ssl-db-ca-root RHN-ORG-TRUSTED-SSL-CERT --ssl-db-cert server.crt --ssl-db-key server.key
----
