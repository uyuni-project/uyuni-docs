...
:description: Monitor Salt events using Saline to collect and analyze metrics from the Salt event bus helping you identify performance issues and improve efficiency
:revdate: 2025-04-07
...
Saline is distributed as a separate container image, `server-saline`, for use with {productname} Server. 
...
* Saline can be deployed during the initial installation or an upgrade of the {productname} server, by using command:

+

----
mgradm install --saline-replicas 1
----
...
ifdef::backend-pdf[]
include::../../../installation-and-upgrade/partials/snippet-ptf-loss-during-upgrade.adoc[]
endif::[]

----
mgradm upgrade && mgradm scale uyuni-saline --replicas 1
----

In all scenarios, the only possible values for `--saline-replicas` are `1` or `0`.

After finishing the deployment `{productname}-saline.service` should be configured on the {productname} Server host system.



== {salt} master configuration recommendations

It is recommended to add `master_stats: True` and adjust the value of `master_stats_event_iter: 60` if needed.
Then restart `salt-master` to get the detailed statistics of internal {salt} calls.
...
To remove Saline from the {productname} Server use `mgradm upgrade` with `--saline-replicas 0`.
