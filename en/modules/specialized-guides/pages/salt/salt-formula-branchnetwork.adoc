...
= Branch Network Formula
:description: Configure the branch servers networking services with DHCP, DNS, TFTP, PXE, and FTP using Salt formulas in the SUSE Multi-Linux Manager Web UI for setup
:revdate: 2025-06-12
...
. In the `Branch Network` section, set these parameters:
...
. Check `Enable Route` if you want the branch server to route traffic from internal LAN to WAN.
...
. In the {susemgr} {webui}, open the details page for the branch server, and navigate to the `Formulas` tab.
. In the `Terminal Naming` section, enter the `Branch Identification` string.
