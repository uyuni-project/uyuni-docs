[[custom-states]]
= Custom Salt States
:description: Create custom Salt states by using centrally managed configuration channels to define and store your own Salt state files on the Server allowing you
:revdate: 2024-08-02
...
The custom states that you create using the {webui} are stored on the {productname} Server in the `/srv/susemanager/salt/<organization>/` directory.
...
. In the `Name` field, type a name for your state.
. In the `Label` field, type a label.
  Use alphanumeric characters, hyphens, and underscores.
  Do not use spaces.
. In the `Description` field, type a short description of the configuration your state performs.
. In the `SLS Contents` field, type the contents of your `init.sls` state.
...
. Copy the key to a file location where the client can access it.
  We recommend saving it in the `/srv/www/htdocs/pub/` directory, where all user public files must be saved.
. In the {productname} {webui}, navigate to menu:Configuration[Channels].
. Click btn:[Create State Channel].
. In the `Name` field, type a name for your state.
...
. In the `SLS Contents` field, create a state to retrieve the appropriate key from the {productname} Server and trust it on the client.
...
This short example maps the `test` state to the system group `12`:
