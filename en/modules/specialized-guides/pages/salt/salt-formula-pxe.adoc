[[pxe-formula]]
= PXE Formula
:description: Configure the PXE boot protocol on your branch server by selecting the PXE formula, setting the kernel filename and PXE root directory parameters, and applying
:revdate: 2025-06-12
...
. Select the `Pxe` formula, and click `Save`.
...
* If the terminals connecting to this branch server are running {arm64} architecture, check the `Enable ARM64 UEFI boot` box.
...
* In the `Kernel Command Line Parameters` field, keep the default value.
