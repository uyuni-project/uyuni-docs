[[salt.terminology]]
= Terminology
:description: Learn the key terms and concepts used in Salt to understand how it manages systems and communicates between clients and the server
:revdate: 2024-12-12
...
[NOTE]
====
To use beacons on {sles} Salt clients, install the `python-pyinotify` package.
For {rhel} systems, install the `python-inotify` package.
====
+
...
Salt modules are stored on clients and the {productname} Server within the `/usr/lib/python*/site-packages/salt/` directory.
...
[WARNING]
====
Do not update the `salt` package using states.
Update all other system packages using states.
You can then update the `salt` package from the {productname} {webui} as a separate step.
====
+
