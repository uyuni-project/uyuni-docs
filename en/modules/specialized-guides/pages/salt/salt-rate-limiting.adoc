[[salt.rate.limiting]]
= Salt Rate Limiting
:description: Configure Salt rate-limiting parameters in the /etc/rhn/rhn.conf configuration file to balance system performance and parallelism by adjusting batch size
:revdate: 2022-01-11
...
Adjust the `java.salt_batch_size` parameter.
...
Adjust the `java.salt_batch_delay` parameter.
