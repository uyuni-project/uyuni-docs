...
:description: Configure custom formulas for the SUSE Multi-Linux Manager Web UI by writing your own custom formulas with forms, and consider directory structures and file
:revdate: 2025-02-06
...
A formula contains two separate directories: `states`, and `metadata`.
...
To create formulas with forms, the metadata directory contains a `form.yml` file.
The `form.yml` file defines the forms for {productname}.
The metadata directory also contains an optional `metadata.yml` file with additional information about the formula.
...
All custom folders must contain a `form.yml` file.
...
The old directory location, `/usr/share/susemanager/formulas`, will continue to work for some time.
You should ensure that you update to the new directory location, `/usr/share/salt-formulas/` as soon as possible.
====
...
{productname} requires a file called `form.yml`, to describe how formula data should look within the {webui}.
The `form.yml` file is used by {productname} to generate the desired formula with forms, with values editable by a user.
...
The formula is called `hd-partitions` and will appear as `Hd Partitions` in the {webui}.
...
This includes the `formulas.sls` file located in `/usr/share/susemanager/formulas/states` or `/usr/share/salt-formulas/states/`.
...
* Formulas must be included for every client listed in the top file.
    Currently this process is initiated by the `mgr_master_tops.py` script which includes the `formulas.sls` file located in `/usr/share/susemanager/formulas/states/` or `/usr/share/salt-formulas/states/`.
    This directory must be a salt file root.
    File roots are configured on the salt-master ({productname}) located at `/etc/salt/master.d/susemanager.conf`.
