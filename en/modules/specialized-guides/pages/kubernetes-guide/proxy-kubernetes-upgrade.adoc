...
:description: Upgrade the proxy on Kubernetes by re-running helm upgrade --install with a newer chart version and --reuse-values to keep existing settings
:revdate: 2026-06-03
...
. Run `helm upgrade` against the new chart version, reusing the values from the previous release:
...
Then add the matching `--set-file global.config=...` / `--set-file global.httpd=...` / `--set-file global.ssh=...` flags to the `helm upgrade` command above.
The same pattern applies to any other value that needs to change: keep `--reuse-values` and overlay the override with `--set`.
====
