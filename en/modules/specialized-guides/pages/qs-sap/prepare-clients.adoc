...
= Preparing Clients
:description: Prepare clients for SAP by installing SUSE Linux Enterprise Server 15 for SAP, registering with SUSE Customer Center, configuring shared storage, and setting
:revdate: 2024-11-29
...
Every client system must have all the other client systems listed in their `/etc/hosts` file.
Open the `/etc/hosts` file on each client, and add the hostname for each of the other clients.
...
This device is used to store files required by HANA, which are located in the `/hana/` directory.
...
Fill in the appropriate details, and make sure you check the `Manage System Completely via SSH` checkbox.
In the `Activation Key` field, select the SLES for SAP activation key.
