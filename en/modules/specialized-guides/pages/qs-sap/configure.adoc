...
= Configure Clients
:description: Configure SAP clients using SUSE Multi-Linux Manager formulas, which include forms for HANA database configuration and cluster setup through YaST tools on the
:revdate: 2025-02-06
...
Navigate to the `Formulas` tab, locate the `Sap Hana Deployment` heading, and check the `Saphanabootstrap` formula in the list.
...
Make sure you check `Install required packages` to install everything you need on the client.
In the `Nodes` sections, type the short hostname of the client to install the HANA database or the hostname you can retrieve on the command line with:
...
Navigate to the `Formulas` tab, locate the `Cluster` heading, and check the `Habootstrap` formula in the list.
Click btn:[Save] and apply the highstate to activate the formula.
