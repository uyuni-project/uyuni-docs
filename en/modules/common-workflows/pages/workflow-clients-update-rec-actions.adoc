...
:description: Automate client updates in SUSE Multi-Linux Manager using recurring actions for efficient maintenance and a supportable state
:revdate: 2025-05-13
...
. As an example we create the action to apply all updates as a recurring action for a system group called **infra-services**.
  In the {productname} {webui} go to menu:Systems[System Groups] and click on `infra-services`.

. Now go to `Recurring Actions` and click btn:[Create].
...
. Assign the states `util.syncall`, `certs`, `channels` and `uptodate` by selecting the checkboxes.
  To perform a reboot afterwards you can also add `reboot` or `rebootifneeded`.

. Save the action by clicking btn:[Save Changes].

. You can edit the execution order of the states.
  The order should be `util.syncall`, `certs`, `channels`, `uptodate` and finally `reboot` or `rebootifneeded` if chosen.
...
. As an example, we create the action to apply the highstate for the same group which was fully updated before.
  In the {productname} {webui}, navigate to menu:Systems[System Groups] and click `infra-services`.
...
. The `uptodate` state applies critical patches to the update components.
...
. {productname} provides the  `reboot` and `rebootifneeded` actions.
...
* For non-transactional {suse} clients, {productname} reboots the client when `zypper -x list-patches` indicates that the patches require a reboot.

* For transactional {suse} clients, {productname} reboots the client if there is a pending transaction.

* For the Red Hat-based clients, {productname} reboots the client if `dnf -q needs-restarting -r` indicates that a reboot is required.
