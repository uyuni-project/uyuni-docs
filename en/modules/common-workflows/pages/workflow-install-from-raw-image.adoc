...
:description: Install using RAW image by downloading the architecture-specific .raw.xz image, preparing the target disk, and transferring the image to the root disk with curl
:revdate: 2025-03-19
...
[role=procedure]
. *Download the Image:* Download the architecture-specific `.raw.xz` image of {productname} Server for your target block size.
. *Make Image Accessible:* Copy the image to a web server (preferred), or expand the `.raw` file to add directly as a disk in a virtual environment (if supported).
. *Target Disk Setup:* If applicable, in virtual machine hypervisor, create:
.. A 40GB root disk
.. Additional storage for persistent {productname} data
...
.. Provide a bootable ISO ({sles} 15 SPX or {sle-micro} 5.5) to access the "Rescue System."
   Add `netsetup=1` on the kernel command line for networking, or configure IP via `ifcfg`.
...
.. Use a disk storage utility like `blockdev` or `lsblk` to verify the block size against your downloaded image.
+
----
...
+
. *Transfer the Image*
.. Use `curl` to copy the `.raw.xz` image onto the root disk:
+
----
curl -Sks $url | xz -d > $device -v
----
+
.. Replace `$url`  with the image URL and `$device` with the root partition device.
...
.. Follow on-screen installation directions and, if needed, register with {scclongform}, adding the {productname} Server Extension.
