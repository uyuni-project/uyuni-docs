...
:description: Learn how to successfully upgrade a registered SUSE Linux Enterprise client pack to a newer product version via Web UI by following detailed instructions
:revdate: 2025-02-06
...
. Log in to {productname} {webui} and navigate to menu:Admin[Setup Wizard > Products] and search for `SUSE Linux Enterprise Server 15 SP5 x86_64`.
...
. Select `SUSE Linux Enterprise Server 15 SP5 x86_64`.
  This will expand further.
. Select `Target Base Channel` as `SLE-Product-SLES15-SP5-Pool for x86_64`.
  Keep `Allow Vendor Change` unchecked.
. Click btn:[Schedule Migration].
  The message will be highlighted `It is better to do a dry run first so continuing with dry run first.`
