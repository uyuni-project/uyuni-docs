...
:description: Install on ppc64le shows how to set up a server using the SelfInstall ISO image on a ppc64le machine by preparing a virtual or physical machine
:revdate: 2025-03-31
...
.. Download the SelfInstall ISO image for the appropriate physical block size (512 or 4096) from suse.com.
   For example: `SUSE-Manager-Server.ppc64le-5.1-SelfInstall-ppc-4096-2024.12.install.iso`
...
.. Select the desired disk for the root partition `(/)`. The system will copy the disk image to the selected volume.
...
+
----
transactional-update register -p Multi-Linux-Manager-Server/5.1/ppc64le -r <REGCODE>
----
.. Apply all available updates using `transactional-update pkg update`.
