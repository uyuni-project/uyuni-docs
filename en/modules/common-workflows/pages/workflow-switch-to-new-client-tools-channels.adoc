...
* The product list must be updated to ensure new client tools are visible. 
  This is done by running `mgr-sync refresh` from the command line, or by navigating to menu:Admin[Setup Wizard > Products] and clicking btn:[Refresh]. 
...
. Select `Unsubscribe` for the old SLE-Manager-Tools channels (or the equivalents for other operating systems).
