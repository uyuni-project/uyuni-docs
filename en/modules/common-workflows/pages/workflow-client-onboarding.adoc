...
= Client Onboarding
:description: Follow the step-by-step procedures for onboarding a client to your SUSE Multi-Linux Manager Server, which involve configuring hostnames, preparing software
:revdate: 2025-02-06
...
. In {yast}, navigate to menu:System[Network Settings] and go to the `Hostname/DNS` tab.
In the `Static hostname` field, type your new hostmame.
...
. In the `Organization Credentials` tab, ensure you have entered your {scclongform} credentials, and are correctly authenticated.
. In the `Products` tab, ensure that the product catalog is fully updated:
