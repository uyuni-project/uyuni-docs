...
When managing a {sles} 15 installation, the BTRFS root file system can become full due to files, caches, or snapshots.
...
The following steps illustrate how to resize a BTRFS LVM partition by adding a new disk (for example, []ath]``/dev/sdb``) to the volume group.
...
[WARNING]
====
Changes to the partition table can have a catastrophic impact on the system. 
