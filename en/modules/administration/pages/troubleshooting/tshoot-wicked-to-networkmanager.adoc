...
=== Step 2: Apply Changes in a Transactional Shell
 
Open a transactional update shell.
All changes made here are staged atomically and only take effect after the reboot in Step 3.
...
NetworkManager and its dependencies are installed from the SL Micro-6.1tig ma repositories.
...
After the system comes back up, run the following checks on the host.
...
See xref:tshoot-wicked-to-networkmanager.adoc#ts-ip-changed[IP Address Changed After Reboot].
