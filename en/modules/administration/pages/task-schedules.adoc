...
Work on a queue with CLM channels to generate differences compared to its original channel and store it into the reporting database.
The queue is feed from the menu:compare-task-default[compare-task-default schedule].
...
Schedules a generic comparison task like performing difference check for clm channel.
