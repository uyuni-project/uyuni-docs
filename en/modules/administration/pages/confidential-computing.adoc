...
[NOTE]
====
`Host Key Document` is public key certificate of the IBM hypervisor server instance where the attested system is going to run.

`Secure Execution Header` is binary file needed to build the IBM secure VM image of the attested system.

Both files should be requested by the adminuistrator creating the attested system VM running on the IBM hypervisor.
====
...
[IMPORTANT]
====
`Host Key Document` and `Secure Execution Header` are not security sensitive, therefore it is safe to stored them on the {productname} server.
====
