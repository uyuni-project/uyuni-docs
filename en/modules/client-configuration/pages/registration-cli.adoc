...
:description: Register clients using the command line by editing the Salt minion configuration file and restarting the service
:revdate: 2026-08-20
...
* requires that you have installed the `venv-salt-minion` (Salt bundle) or, in the case of {sle} 15 or its derivatives, the `salt-minion` package on the Salt client before registration.
...
[IMPORTANT]
====
The plain `salt-minion` is unsupported in general.
Use this section only for bootstrapping {sle} 15 or its derivatives. It is replaced by `venv-salt-minion` during bootstrapping.
====
