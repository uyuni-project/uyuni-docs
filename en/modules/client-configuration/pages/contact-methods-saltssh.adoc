...
:description: Configure SSH Push to allow the Server to securely connect to clients in a DMZ by establishing a reverse tunnel that bypasses firewall restrictions
:revdate: 2026-08-24
...
. Configure passwordless `sudo` access on the client for the unprivileged user.
  For detailed instructions, see xref:client-configuration:registration-sudo.adoc[].
...
. After changing this configuration parameter you must restart services such as `salt-secrets-config.service`, `tomcat.service`, and `taskomatic.service`.
  To cover all needed services, restart the {productname} Server.
  Outside of the container, from the container host, run:
+
----
mgradm restart
----
