...
The Salt `virt` module is available within the Salt Bundle (`venv-salt-minion`) on SUSE-based operating systems ({sles}, {slemicro}, {opensuse} Leap, and {opensuse} Tumbleweed). To use the module, you must install the system `libvirt` library dependencies, which are not included in the bundle by default.
...
To use the Salt `virt` module (for example, on physical virtual hosts or hypervisors like KVM) with the Salt Bundle on {sle} and {opensuse} systems:
