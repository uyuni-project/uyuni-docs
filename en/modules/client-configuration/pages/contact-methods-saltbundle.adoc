...
The Salt `virt` module is available within the Salt Bundle (`venv-salt-minion`) on SUSE-based operating systems (SUSE Linux Enterprise Server, SUSE Linux Enterprise Micro, openSUSE Leap, and openSUSE Tumbleweed). To use the module, you must install the system `libvirt` library dependencies, which are not included in the bundle by default.

On other operating systems (such as Red Hat Enterprise Linux or Debian-based systems), the Salt `virt` module is not available or supported inside the Salt Bundle.

To use the Salt `virt` module (for example, on physical virtual hosts or hypervisors like KVM) with the Salt Bundle on SUSE Linux Enterprise and openSUSE systems:
