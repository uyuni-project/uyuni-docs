...
Many operating systems disable SSH access for the `root` user by default, or are installed without root access enabled.
To onboard these clients, you must edit the `sudoers` file on the client system.

For detailed instructions on configuring `sudo` for non-root onboarding, see xref:client-configuration:registration-sudo.adoc[].
...
For more information, see https://documentation.suse.com/multi-linux-manager/5.1/api/docs/api/system.html#apidoc-system-obtainReactivationKey-loggedInUser-sid.
