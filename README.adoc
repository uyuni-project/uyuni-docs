...
Each page in `en/modules/*/pages/` has the date of its last change.
Two attributes below the document title give this date:
...
When you change a page, set `:revdate:` to the date of your change.
A pull request fails if a page that it changes:
...
Use these attributes on pages only.
Do not add them to a file in `partials/`.
An attribute entry in an included file replaces the same attribute in the page that includes it.
A `revdate` in a partial therefore replaces the date of each page that includes it.
...
This command uses https://docs.astral.sh/uv/[uv] if you have it, and `python3` if you do not.
If you have neither, use a container:
...
This repository contains the official Uyuni and SUSE Multi-Linux Manager 5.1 documentation.
...
* link:docs/local-toolchain-setup.md[docs/local-toolchain-setup.md] — Local toolchain build (Task, Go, Antora, Asciidoctor PDF)
