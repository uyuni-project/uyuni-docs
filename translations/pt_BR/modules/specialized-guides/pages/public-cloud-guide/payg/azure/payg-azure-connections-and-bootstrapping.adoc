...
{payg} ({payglongform}) Images:: Connecting and registering {payg} images is supported and can be done without additional prerequisites.

{byos} ({byoslongform}) Images:: Registering {byos} images requires {scc} ({scclongform}) credentials beforehand. If a user attempts to onboard a {byos} image without providing {scc} credentials, the operation will fail, as these credentials are required to verify your subscription.
...
Inbound and outbound connections to {azure} are released after 4 minutes of being idle. This affects minion connectivity, especially when your {productname} server is in {azure} and you have some on-premises minions. If a connection is idle for more than 4 minutes, connectivity is lost.
