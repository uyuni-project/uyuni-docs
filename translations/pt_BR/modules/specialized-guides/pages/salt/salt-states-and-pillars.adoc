...
You can create custom Salt states with {productname}. For more information, see xref:specialized-guides:salt/salt-custom-states.adoc[].
...
{productname} exposes a small amount of internal data as pillars which can be used with custom states. Pillars are created on the {productname} Server, and contain information about a client or group of clients. For custom information in pillars, see xref:client-configuration:custom-info.adoc[]. Pillars are useful for sensitive data, configuration of clients, variables, and any arbitrary data.
