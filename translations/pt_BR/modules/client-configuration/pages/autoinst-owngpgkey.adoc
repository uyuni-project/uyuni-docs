...
. To copy GPG keys to the container use [command]``mgradm gpg add``. This command copies the GPG key and adds it to the customer keyring.
. Run command [command]``mgradm restart``. This command creates a unique keyring using the SUSE and the customer keyring.
