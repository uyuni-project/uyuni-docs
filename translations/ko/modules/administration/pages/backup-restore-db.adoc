...
This guide describes how to manage online database backups for the {productname} server using [command]``mgradm``. The backup system is based on {postgresql} continuous archiving Write-Ahead Logging (WAL), similarly like [command]``smdba`` in previous versions.
...
Only full recovery is supported by [command]``mgradm`` tool. The restore procedure will restore everything from the backup location.
====
