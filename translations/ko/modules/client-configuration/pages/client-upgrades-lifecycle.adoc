...
. {productname} {webui}에서 menu:시스템[시스템 목록]으로 이동하여 프로비저닝할 클라이언트를 선택하고 선택한 클라이언트에 시스템 세트 관리자를 추가합니다.
...
PXE를 사용할 수 있는 클라이언트의 경우 마이그레이션은 클라이언트를 조달하자마자 자동화됩니다. 기타 모든 클라이언트의 경우에는 {cobbler}를 사용해 업그레이드를 수행할 수 있습니다.



.절차: {cobbler}를 사용한 클라이언트 업그레이드
