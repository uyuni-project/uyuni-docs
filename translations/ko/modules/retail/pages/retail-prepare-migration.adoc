...
It is possible to run a SLEPOS Admin server and {productname} Server in parallel. In such a scenario, branches that have been migrated will run on the {productname} server, while those that have not yet been migrated can continue to run on the SLEPOS Admin server. This includes all operations, such as adding new terminals, or building and deploying new images.
