...
----
+
You can configure this using the PXE formula. For more information, see xref:salt:formula-pxe.adoc[].
* For ``ftp``, you can use an A record that resolves to an IP address instead of a CNAME. Alternatively, you can change the terminal boot process using Salt pillars. For more information, see xref:retail-deploy-terminals.adoc[].
