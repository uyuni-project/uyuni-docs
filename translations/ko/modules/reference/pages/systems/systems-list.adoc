...
The menu:Systems[System List > Non Compliant] section lists every client that has non-compliant packages installed. Packages are considered non-compliant if they have been installed from a source other than {productname}. In the list, the [guimenu]``Extra Packages`` column shows the number of non-compliant packages on the client. If there is at least one non-compliant package, you can click the number to display listings of those non-compliant packages per system.
...
The menu:Systems[System List > Inactive] section lists every client that has not checked in with the {productname} Server in 24 hours or more.
