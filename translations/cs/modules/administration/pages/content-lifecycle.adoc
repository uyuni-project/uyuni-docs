...
. In the [guimenu]``Prefix`` field, type a name prefix. This value will be prepended to the name of every filter created by the template. If the template is being applied in the context of a project, this field will be prefilled with the project label.
...
=== Moduly AppStream s výchozími hodnotami
...
[IMPORTANT]
====
Since {rhel}{nbsp}9, modules do not have any defined default streams. Therefore, using this template with {rhel}{nbsp}9 sources will have no effect.
====
