...
On your client, open the [path]``/etc/salt/minion.d/custom.conf`` or [path]``/etc/venv-salt-minion/minion.d/custom.conf`` configuration file. Add this line to enter the master's fingerprint replacing the example fingerprint:
+
