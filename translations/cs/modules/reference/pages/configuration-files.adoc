...
When a new configuration file is created with {productname}, a backup of the previous file, including its full path, is stored in [path]``/var/lib/rhncfg/backups/``. The backup retains its filename but has a _.rhn-cfg-backup_ extension.
