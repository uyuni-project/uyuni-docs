...
There are two tools for synchronizing clients to the server. For clients that are connected to the {scc}, use ``mgr-sync``. For all other clients, use ``spacewalk-repo-sync``.
