...
The initial RAM disk ([path]``initrd``) used by PXE boot process normally contains {suse}'s GPG key only. We need to add our own key to this file, so it can be used to check the packages.
...
You can modify an installation image with the [command]``mksusecd`` utility. This command is contained in the Development Tools module.
