...
The menu:Audit[SCAP Policies] section displays the compliance policies defined in your organization. Each policy ties together a SCAP content file, an XCCDF profile, and optional tailoring to define a compliance standard.

Click btn:[Create] to create a new compliance policy. For more information about creating policies, see <<ref-scap-policy-create>>.

Each row in the list provides btn:[Details], btn:[Edit], and btn:[Delete] action buttons. Use btn:[Details] to view the policy summary, configuration, and scan history. Use btn:[Edit] to modify the policy. Use btn:[Delete] to remove the policy. A confirmation dialog is displayed before deletion.
...
[WARNING]
====
Enabling btn:[Fetch Remote Resources] requires internet access and significant available memory on the target minion. Ensure the target system has sufficient resources before enabling this option.
====
...
The menu:Audit[SCAP Policies > Edit] page allows you to update an existing compliance policy. All fields from the creation form can be modified.

Click btn:[Update] to save the changes. Click btn:[Back to List] to return to the policies list without saving.
...
The scan history table shows all scans performed using this policy. Use the checkbox btn:[Show only latest scan per system] to filter the table to display only the most recent scan for each system.
