...
2. Delete all files and directories that are not allowed in
...
6. Write the DHCP files if management is enabled (unsupported).  For more
...
10. Run post-sync triggers.  This can be any number of shell scripts
    (unsupported).
...
Long scripts or strings can be escaped by wrapping them with the `\#raw` and
`\#end raw` directives.  For example:
