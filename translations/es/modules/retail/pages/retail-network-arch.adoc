...
The branch server and the terminals are connected to the same network as the {productname} server. In this configuration, the branch server is not required to manage a network (DHCP and DNS services), but acts as a PXE boot server and provides FTP and TFTP services.
