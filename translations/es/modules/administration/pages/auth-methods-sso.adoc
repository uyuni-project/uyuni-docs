...
[IMPORTANT]
====
The mapping between the IdP user and the {productname} user is specified in a SAML:Attribute. The SAML:Attribute must be configured in the IdP and must be passed to {productname} in the SAML authentication. The attribute must be named ``uid`` and must contain the {productname} user mapped to it after login. The {productname} must be created before you activate single sign-on.
====
