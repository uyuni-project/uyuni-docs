...
. In the {productname} {webui}, navigate to menu:Systems[Systems List], select
  ...
  you can assign clients through the system set manager by navigating to
  menu:Systems[System Set Manager] and using the menu:Misc[Maintenance
  Windows] tab.
...
previously scheduled actions that conflict with the new maintenance
schedule.
====
