...
Proceed as follows to solve the problem with resizing the storage medium. Run all listed commands as [literal]``root`` on the container host.

.Procedure: Resize Storage Medium

. Increase the disk size. Actions to take depend on the installation scenario.
...
. Resize the filesystem. For example, for an XFS filesystem, run the command:
