...
{saltboot} redeployment grains have no side effects, and do not require any further configuration. The limitation is that terminal must be accessible by [systeminfo]``salt``.
...
[NOTE]
====
Creating [systemitem]``saltboot_force_redeploy`` custom key is a one time operation. When created, it is available for repeated use.
====
...
An API call [systemitem]``system.setPillar`` can be used to set specific {saltboot} options through {salt} pillar data. {productname} {saltboot} integration is using [systemitem]``tuning-saltboot`` pillar category to manage {saltboot} tuning, including forced redeployment or disk repartition. Using this pillar category allows {productname} to reset {saltboot} flag once the terminal is booted up.
...
[WARNING]
====
Pillars specified outside of {productname} database cannot be reset automatically. Without manual intervention, the terminal will download a new image on each reboot.
====
