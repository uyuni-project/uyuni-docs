...
Make sure you check [guimenu]``Install required packages`` to install everything you need on the client. In the [guimenu]``Nodes`` sections, type the short hostname of the client to install the HANA database or the hostname you can retrieve on the command line with:
