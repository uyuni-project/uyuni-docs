...
A role can be assigned to a running instance through the following steps: menu:Actions[Security > Modify IAM-role]. It needs to have the following policy attached in order to be used:
...
. Select the AMI image and provide a meaningful **name** for your server. Review the details and pricing information provided.
...
.. If an IAM role has been previously created, you can attach it by selecting it from the existing menu:IAM role[] drop-down. Otherwise, attach this role after the instance has been launched.
