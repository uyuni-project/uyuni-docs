...
Minions::
Salt clients that are connected to and controlled by the {salt} master on the {productname} Server. In {productname}, these are sometimes referred to as {salt} clients. This is a difference in terminology only.

Modules::
Functions within Salt are stored in modules. Salt modules are stored on clients and the {productname} Server within the [path]``/usr/lib/python*/site-packages/salt/`` directory. There are many types of Salt modules, including state and execution modules. You can write your own Salt modules using Python.
+
