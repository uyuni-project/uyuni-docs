...
本文假设代理主机系统已完成引导、已订阅基础操作系统通道（如 {sles} 15 SP7 或 {sl-micro} 6.1）以及代理扩展通道。
...
** {productname} Proxy Extension 5.1（需匹配体系结构）
...
您可通过 {webui} 安装这些软件包：导航到[literal]``软件`` > [literal]``软件包`` > [literal]``安装``选项卡，然后搜索上述软件包并完成安装。
...
*** 在[literal]``仓库 URL`` 中填写 [literal]``registry.suse.com/suse/multi-linux-manager/5.1/x86_64``。

*** 从下拉列表中选择标记。

** 如果选择[literal]``高级``，表单会显示一个额外的部分：

*** 对于每个独立容器的 URL 字段，均需填写仓库地址 [literal]``registry.suse.com/suse/multi-linux-manager/5.1/x86_64``，并添加对应的后缀（例如 `_proxy-httpd_` 或 `_salt-broker_`）。
