...
对于 Redfish，请确保您可以在客户端和 {productname} 服务器之间建立有效的 SSL 连接。您必须信任用于对 Redfish 管理控制器的 SSL 服务器证书签名的证书颁发机构。CA 证书必须为 [literal]``.pem`` 格式，并存储在 {productname} 服务器上的 [path]``/etc/pki/trust/anchors/`` 中。保存证书后，请运行 [command]``update-ca-certificate``。
