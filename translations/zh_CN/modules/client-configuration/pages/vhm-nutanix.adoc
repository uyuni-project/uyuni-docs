...
[NOTE]
====
有时，在浏览器中使用 HTTPS 连接 Nutanix Prism API 服务器可能会发生``证书无效``错误。如果发生此情况，刷新来自虚拟主机管理器的数据将会失败。Nutanix API 服务器上必须存在有效的 SSL 证书（不是自我签名证书）。如果您为 Nutanix SSL 证书使用了自定义 CA 机构，请将自定义 CA 证书复制到 {productname} 服务器上的 [path]``/etc/pki/trust/anchors`` 中。
